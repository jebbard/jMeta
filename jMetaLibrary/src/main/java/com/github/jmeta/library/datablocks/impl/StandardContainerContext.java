...
/**
 * {@link StandardContainerContext} contains contextual bits of metadata for the
 * current container like sizes, counts, byte orders etc. of fields or other
 * data blocks within this container. Saying this, during parsing e.g. header or
 * footer fields, this context of metadata is built up, and
 * {@link StandardContainerContext} allows parsing code to later access this
 * information.
 *
 * {@link StandardContainerContext}s are hierarchical in a way that they contain
 * a reference to the {@link StandardContainerContext} of the parent container
 * (if any). If a size could not be determined, the delegate to the parent
 * {@link StandardContainerContext}.
 */
public class StandardContainerContext implements ContainerContext, DataBlockEventListener {

	/**
	 * {@link FieldCrossReference} combines a {@link Field} containing a field
	 * function with the {@link DataBlockId} referenced by it for easier access by
	 * the {@link FieldFunctionStore} and {@link ContainerContext}.
	 *
	 * @param <T> The interpreted field type
	 * @param <F> The concrete type of {@link AbstractFieldFunction}
	 */
	public static class FieldCrossReference<T, F extends AbstractFieldFunction<T>> {
		 ...
		 *
		 * @param referencedBlock          The referenced {@link DataBlockId}, must not
		 *                                 be null
		 * @param referencingField         The {@link Field} referencing the referenced
		 *                                 {@link DataBlockId}, must not be null
		 * @param referencingFieldFunction The {@link AbstractFieldFunction} the field
		 *                                 uses to reference the data block, must not be
		 *                                 null
		 */
	...
	/**
	 * {@link FieldFunctionStore} stores all {@link FieldCrossReference}s for a
	 * concrete type of {@link AbstractFieldFunction}.
	 *
	 * @param <T> The interpreted field type
	 * @param <F> The concrete type of {@link AbstractFieldFunction}
	 */
	private class FieldFunctionStore<T, F extends AbstractFieldFunction<T>> {
		 ...
		 *
		 * @param fieldFunctionClass The concrete class of
		 *                           {@link AbstractFieldFunction}, must not be null
		 */
		...
		/**
		 * Adds the field function of the given field and the type represented by this
		 * {@link FieldFunctionStore}.
		 *
		...
		/**
		 * Returns the {@link FieldCrossReference} for the given target id and sequence
		 * number or null if there is none.
		 *
		 * @param targetId       The target {@link DataBlockId}, must not be null
		 * @param sequenceNumber The sequence number, must not be negative
		 * @return the {@link FieldCrossReference} for the given target id and sequence
		 *         number or null if there is none
		 */
	 ...
	 *
	 * @param spec                   The {@link DataFormatSpecification} of this
	 *                               context, must not be null
	 * @param parentContainerContext The parent {@link ContainerContext}, might be
	 *                               null if this {@link ContainerContext} belongs
	 *                               to a top-level container
	 * @param container              TODO
	 * @param customSizeProvider     A custom {@link SizeProvider} implementation to
	 *                               be used or null if none
	 * @param customCountProvider    A custom {@link CountProvider} implementation
	 *                               to be used or null if none
	 */
	...
	public ByteOrder getByteOrderOf(DataBlockId id, int sequenceNumber) {
		...
		if (crossReference == null) {
			...
			while (currentId != null) {
				FieldCrossReference<String, ByteOrderOf> parentCrossReference = byteOrders.getCrossReference(currentId,
					0);
	...
	public long getSizeOf(DataBlockId id, int sequenceNumber) {
		...
		if (sizeCrossRef == null) {
			...
			if (summedSizeCrossReference != null) {
				Set<DataBlockId> allTargetIds = summedSizeCrossReference.getReferencingFieldFunction()
					.getReferencedBlocks().stream().map(DataBlockCrossReference::getId).collect(Collectors.toSet());
