...
public class StandardDataFormatSpecification implements DataFormatSpecification {
	 ...
	 * @param defaultNestedContainerId      TODO
	 */
	...
	private void initGenericIdPatterns(Set<DataBlockId> genericDataBlocks) {
		...
		for (Iterator<DataBlockId> iterator = genericDataBlocks.iterator(); iterator.hasNext();) {
			...
			Matcher matcher = StandardDataFormatSpecification.GENERIC_PLACEHOLDER_PATTERN.matcher(idString);
			...
			while (matcher.find()) {
				...
				idPattern.append(StandardDataFormatSpecification.GENERIC_ID_REPLACE_PATTERN);
			}
	...
	*/
	...
	/**
	 * Validates all field functions and ensures the dynamic occurrence / size data
	 * blocks have a fitting field function
	 */
	@SuppressWarnings("unchecked")
		...
		for (DataBlockDescription dataBlockWithDynamicOccurrences : dataBlocksWithDynamicOccurrences) {
			...
			if (dataBlockWithDynamicOccurrences.isOptional()) {
				if (!hasFieldFunctionForId
					|| !hasFieldFunctionOfType(fieldFunctionsByTargetId.get(id), PresenceOf.class)) {
					throw new InvalidSpecificationException(
						InvalidSpecificationException.VLD_FIELD_FUNC_OPTIONAL_FIELD_PRESENCE_OF_MISSING,
						dataBlockWithDynamicOccurrences);
				}
			} else {
				if (!hasFieldFunctionForId
					|| !hasFieldFunctionOfType(fieldFunctionsByTargetId.get(id), CountOf.class)) {
					throw new InvalidSpecificationException(
						InvalidSpecificationException.VLD_FIELD_FUNC_DYN_OCCUR_COUNT_OF_MISSING,
						dataBlockWithDynamicOccurrences);
				}
			}
		}

		// NOTE: It would be an idea to check that for every block with dynamic size,
		// there is a SizeOf or SummedSizeOf
		// field function. However, this turns out to be very complex, as this is
		// incomplete: (1) Terminated fields exist.
		// (2) Sometimes the size of a data block (e.g. field-based payload) is just
		// given by the summed size of its
		// children.
		// (3) Sometimes the size of a data block is implicitly given by the known size
		// of its parent. (4) Sometimes the
		// size calculations are so complex you need custom code for it, deriving the
		// size from a multitude of header
		// fields (see e.g. MP3).
	}
	...
	private void validateTopLevelMagicKeys() {
		for (Iterator<DataBlockDescription> iterator = m_topLevelDataBlocks.iterator(); iterator.hasNext();) {
			...
			if (dataBlockDesc.getHeaderMagicKeys().isEmpty()) {
				throw new InvalidSpecificationException(InvalidSpecificationException.VLD_MAGIC_KEY_MISSING,
					dataBlockDesc);
			}
		}
	}
