...
/**
 * {@link Flags} provides support for reading and writing flag values. A flag is
 * either a single bit that can be set (=1) or not set (=0) within a number of
 * bytes, or a series of multiple bits that can be assigned more than two values
 * (so called multi-bits). Each flag has a meaning which is expressed by its
 * name in the {@link FlagSpecification} belonging to the flags. The
 * {@link FlagSpecification} defines the byte length, order, position and bit
 * size of flags. It does not change over the whole lifetime of a {@link Flags}
 * object.
 *
 * Flag values can be written by using {@link #setFlag(String, boolean)} or by
 * specifying a byte value that e.g. comes from a file.
 */
public class Flags {
	 ...
	 *
	 * @param spec The {@link FlagSpecification} defining names, length and
	 *             positions of the flags. Must not be null
	 */
	...
	/**
	 * Returns a byte representation of the {@link Flags}. If the flags consist of
	 * multiple bytes, this only returns the first byte.
	 *
	...
	/**
	 * Returns an int representation of the {@link Flags}. If the flags consist of
	 * more than {@link #INT_BYTE_LENGTH} bytes, this only returns the first four
	 * bytes.
	 *
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >=}{@link #LONG_BYTE_LENGTH}
	 * for {@link #getSpecification()}
	 * 
	 * @return An int representation of the {@link Flags}.
	 */
	public int asInt() {
		Reject.ifFalse(getSpecification().getByteLength() >= Flags.INT_BYTE_LENGTH,
	 ...
	 */
	public long asLong() {
		Reject.ifFalse(getSpecification().getByteLength() >= Flags.LONG_BYTE_LENGTH,
	...
	/**
	 * Returns a short representation of the {@link Flags}. If the flags consist of
	 * more than {@link #SHORT_BYTE_LENGTH} bytes, this only returns the first two
	 * bytes.
	 * 
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >=}
	 * {@link #SHORT_BYTE_LENGTH} for {@link #getSpecification()}
	 *
	 * @return A short representation of the {@link Flags}.
	 */
	public short asShort() {
		Reject.ifFalse(getSpecification().getByteLength() >= Flags.SHORT_BYTE_LENGTH,
	...
	/**
	 * Writes the flags from a byte array. Flag positions that are not actually
	 * occupied by a named flag are ignored. Flags are set or unset according to the
	 * state of the bit at the specified position. If the flags consist of more than
	 * the arrays byte length bytes, this only writes the array bytes.
	 *
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >= array.length} for
	...
	/**
	 * Writes the flags from a byte value. Flag positions that are not actually
	 * occupied by a named flag are ignored. Flags are set or unset according to the
	 * state of the bit at the specified position. If the flags consist of multiple
	 * bytes, this only writes the first byte.
	 *
	...
	/**
	 * Writes the flags from an int value. Flag positions that are not actually
	 * occupied by a named flag are ignored. Flags are set or unset according to the
	 * state of the bit at the specified position. If the flags consist of more than
	 * {@link #INT_BYTE_LENGTH} bytes, this only writes the first four bytes.
	 *
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >=}{@link #LONG_BYTE_LENGTH}
	 * for {@link #getSpecification()}
	...
	/**
	 * Writes the flags from a long value. Flag positions that are not actually
	 * occupied by a named flag are ignored. Flags are set or unset according to the
	 * state of the bit at the specified position.
	 *
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >=}{@link #LONG_BYTE_LENGTH}
	 * for {@link #getSpecification()}
	...
	/**
	 * Writes the flags from a short value. Flag positions that are not actually
	 * occupied by a named flag are ignored. Flags are set or unset according to the
	 * state of the bit at the specified position. If the flags consist of more than
	 * {@link #SHORT_BYTE_LENGTH} bytes, this only writes the first two byte.
	 *
	 * The specified byte length must be greater or equal to required byte length -
	 * {@link FlagSpecification#getByteLength()}{@literal >=}
	 * {@link #SHORT_BYTE_LENGTH} for {@link #getSpecification()}
	...
	/**
	 * Determines if a specific flag is set in the current byte value of this
	 * {@link Flags}.
	 *
	 * @param flagName The name of the flag. Must be defined in the specification.
	 *                 Must be defined in the flag specification
	 *
	 * @return true if the flag is set, false if it is not set. For multi-bit flags,
	 *         this method returns true if the at least one bit is set in the
	 *         multi-bit flag.
	 */
	...
	/**
	 * Returns the signed integer representation of the flag value. For single-bit
	 * flags, the method returns 1 if the single-bit is set and 0 otherwise.
	 *
	 * @param flagName The name of the flag. Must be defined in the specification.
	...
	public int getFlagIntegerValue(String flagName) {
		...
		int spannedBytes = 1 + (bitCountInFurtherBytes / Byte.SIZE)
			+ ((bitCountInFurtherBytes % Byte.SIZE) == 0 ? 0 : 1);

		int currentBitCount = 0;

		// The indices of the start and end bit belonging to the flag in the current
		// byte
		int startBitIndex = flagStartAddress.getBitPosition();
		int endBitIndex = bitCountInFurtherBytes == 0
			? (flagStartAddress.getBitPosition() + description.getBitSize()) - 1
			: Byte.SIZE - 1;
		...
		for (int i = flagStartAddress.getByteAddress(); i < (flagStartAddress.getByteAddress() + spannedBytes); i++) {
			...
			// Mask the flags (extracting the relevant bits only) and prevent signed
			// one-bits
	 ...
	 * 
	 * @param flagName The name of the flag. Must be defined in the flag
	 *                 specification
	...
	/**
	 * Returns the {@link FlagSpecification} associated with this {@link Flags}
	 * object.
	 *
	 * @return The {@link FlagSpecification} associated with this {@link Flags}
	 *         object.
	 */
	 ...
	 *
	 * @param flagName The flag to set. Must be defined in the flag specification
	 * @param set      true to set the flag, false to unset the flag.
	 */
	...
	/**
	 * Sets the flag with the given name to the given integer value. As a
	 * precondition, the given value must not be bigger than 2^(bit size of the
	 * flag). For single-bit flags, only 0 (=cleared) and 1 (=set) may be passed.
	 *
	 * @param flagName The name of the flag. Must be defined in the specification.
	 * @param value    The new value of the flag in a signed integer representation.
	 *                 Each bit in the integer corresponds to one bit in the flag.
	 *                 Therefore the given value must not be bigger than 2^(bit size
	 *                 of the flag).
	 */
