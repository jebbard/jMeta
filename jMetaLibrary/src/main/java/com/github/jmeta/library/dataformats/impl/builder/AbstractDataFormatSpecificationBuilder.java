...
/**
 * {@link AbstractDataFormatSpecificationBuilder} is the base class of all
 * {@link DataBlockDescriptionBuilder}s. It allows to create
 * {@link DataBlockDescription}s and offers a generic {@link #finish()} method.
 * On top of this, it provides default implementations for all Methods of
 * {@link DataBlockDescriptionBuilder} and {@link DataFormatBuilder}.
 *
...
public abstract class AbstractDataFormatSpecificationBuilder<P extends DataFormatBuilder, C extends DataBlockDescriptionBuilder<C>>
	...
	// We use a map here to ensure we cannot add children with the same id multiple
	...
	public AbstractDataFormatSpecificationBuilder(P parentBuilder, String localId, String name, String description,
		...
		Reject.ifTrue(localId.contains(DataBlockId.SEGMENT_SEPARATOR),
			"localId.contains(DataBlockId.SEGMENT_SEPARATOR)");
	 ...
	 * {@link DynamicOccurrenceBuilder#asOptional()} without claiming to override it
	 * (as it comes deeper down into the interface hierarchy). This way, it is
	 * possible to provide the occurrence methods just for a selected group of
	 * builders only.
	 */
	public C asOptional() {
	...
	/**
	 * Allows derived builders to remove a child {@link DataBlockDescription} with a
	 * given id
	 *
	 * @param childId The {@link DataBlockId} of the child to remove, must not be
	 *                null
	 */
	...
	/**
	 * Allows to set or unset the container {@link DataBlockDescription} built by
	 * this class as default nested container.
	 *
	 * @param isDefaultNestedContainer true for setting it as default nested
	...
	/**
	 * Allows to set {@link FieldProperties} for the field
	 * {@link DataBlockDescription} built by this class.
	 *
	 ...
	 *
	 * @param minimumByteLength The minimum byte length
	 * @param maximumByteLength The maximum byte length
	 */
