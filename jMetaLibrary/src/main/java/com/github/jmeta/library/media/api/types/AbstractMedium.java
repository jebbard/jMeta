...
/**
 * {@link AbstractMedium} represents a physical medium where binary data can be
 * stored.
 *
...
public abstract class AbstractMedium<T> implements Medium<T> {
	...
	/**
	 * Creates a new {@link AbstractMedium} with default values used for all
	 * configuration values, see {@link #getMaxCacheSizeInBytes()} and
	 * {@link #getMaxReadWriteBlockSizeInBytes()}.
	 *
	 * @param medium         see
	 *                       {@link #AbstractMedium(Object, String, boolean, boolean, long, int)}
	 * @param name           see
	 *                       {@link #AbstractMedium(Object, String, boolean, boolean, long, int)}
	 * @param isRandomAccess see
	 *                       {@link #AbstractMedium(Object, String, boolean, boolean, long, int)}
	 * @param isReadOnly     see
	 *                       {@link #AbstractMedium(Object, String, boolean, boolean, long, int)}
	 */
	public AbstractMedium(T medium, String name, boolean isRandomAccess, boolean isReadOnly) {
		this(medium, name, isRandomAccess, isReadOnly, Medium.DEFAULT_MAX_CACHE_SIZE_IN_BYTES,
			Medium.DEFAULT_MAX_READ_WRITE_BLOCK_SIZE_IN_BYTES);
	}

	/**
	 * Creates a new {@link AbstractMedium}, using explicit values for all
	 * properties.
	 *
	 * @param medium                       The underlying raw medium object
	 * @param name                         An externally known name of the
	 *                                     {@link AbstractMedium} that must be used
	 *                                     as medium name. May be null to indicate
	 *                                     that only the physical name of the
	 *                                     {@link AbstractMedium} is relevant.
	 * @param isRandomAccess               true if the medium is random-access,
	 *                                     false otherwise
	 * @param isReadOnly                   true to make this a read-only
	 *                                     {@link Medium}, false otherwise.
	 * @param maxCacheSizeInBytes          see {@link #getMaxCacheSizeInBytes()}
	 ...
	 *
	 * @param newMedium The new medium content.
	 */
