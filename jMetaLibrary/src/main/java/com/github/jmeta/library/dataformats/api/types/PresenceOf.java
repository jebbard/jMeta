...
/**
 * {@link PresenceOf} expresses that the field it refers tells whether the
 * target data block id is present or not.
 */
public class PresenceOf extends AbstractFieldFunction<Flags> {
	 ...
	 * @param flagName        The name of the flag indicating presence or absence,
	 *                        must not be null
	 * @param flagValue       The flag's value indicating presence, if the value
	 *                        differs, this indicates absence
	 */
	...
	public void validate(DataBlockDescription fieldDesc, DataFormatSpecification spec) {
		...
		if (!fieldDesc.getFieldProperties().getFlagSpecification().hasFlag(flagName)) {
			throw new InvalidSpecificationException(
				InvalidSpecificationException.VLD_FIELD_FUNC_PRESENCE_OF_UNSPECIFIED_FLAG_NAME, fieldDesc, flagName);
		}
	}
