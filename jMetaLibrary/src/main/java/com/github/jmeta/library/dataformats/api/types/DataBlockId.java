...
/**
 * Identifies a data block within its {@link DataFormatSpecification}. Ids are
 * hierarchical in a sense that they consist of segments, and each segment is
 * called a local id. It is a string that contains only alphanumerical
 * characters. A {@link DataBlockId} is called the child another one if it just
 * appends one or several local id segments to it. The other {@link DataBlockId}
 * is then called its parent.
 */
public class DataBlockId {
	...
	/**
	 * A default local id for a flags {@link PhysicalDataBlockType#FIELD} data
	 * block.
	 */
	...
	private static String computeGlobalId(List<String> segmentList) {
		return segmentList.stream().collect(Collectors.joining(DataBlockId.SEGMENT_SEPARATOR));
	}
	 ...
	 *
	 * @param dataFormat The {@link ContainerDataFormat} this {@link DataBlockId}
	 *                   belongs to, must not be null
	...
	public DataBlockId(ContainerDataFormat dataFormat, List<String> segments) {
		...
		globalId = DataBlockId.computeGlobalId(idSegments);
	 ...
	 * @param dataFormat The {@link ContainerDataFormat} this {@link DataBlockId}
	 *                   belongs to, must not be null
	 * @param globalId   The global id, must not be null
	 */
	public DataBlockId(ContainerDataFormat dataFormat, String globalId) {
		...
		this.dataFormat = dataFormat;
		idSegments.addAll(Arrays.asList(globalId.split(Pattern.quote(DataBlockId.SEGMENT_SEPARATOR))));
		parentId = parentIdFromSegments(idSegments);
	}

	/**
	 * Creates a new {@link DataBlockId} starting from the parent
	 * {@link DataBlockId} and the local id.
	 *
	 * @param parentId The parent {@link DataBlockId}, must not be null
	 * @param localId  The local id, must not be null
	 */
	...
	/**
	 * @return the id segments, ordered hierarchically from top-level parent down to
	 *         the leaf child
	 */
	...
	/**
	 * @return the parent {@link DataBlockId} or null if this is a top-level
	 *         {@link DataBlockId}
	 */
