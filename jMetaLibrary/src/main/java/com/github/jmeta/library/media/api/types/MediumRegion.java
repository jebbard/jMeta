...
/**
 * {@link MediumRegion} represents a part of an external medium. This part can
 * either be cached or not. If it is cached, it additionally contains a sequence
 * of bytes stored in the region, i.e. bytes e.g. previously read from an
 * {@link Medium}. This class does not support cases where the number of cached
 * bytes does not equal the size of the region. Saying this, if it contains
 * cached data, the number of bytes always exactly equals the size of the
 * region. Either all or nothing is cached.
 *
 * A {@link MediumRegion} may at maximum have a size of
 * {@link Integer#MAX_VALUE}. However, the minimum size is 0, so the idea of the
 * "empty" region is supported.
 *
 * Note that - although {@link #calculateEndOffset()} and
 * {@link MediumOffset#advance(long)} could have been used more to write even
 * more readable algorithm, intentionally, they are avoided to optimize for new
 * {@link MediumOffset} only to be created if really necessary.
 */
public class MediumRegion {

	/**
	 * {@link MediumRegionClipResult} represents the result of clipping two
	 * overlapping {@link MediumRegion}s against each using
	 * {@link MediumRegion#clipOverlappingRegions(MediumRegion, MediumRegion)}.
	 */
	public static class MediumRegionClipResult {
		 ...
		 * 
		 * @param overlappedPartOfLeftRegion           See getter for details, must not
		 *                                             be null
		 * @param nonOverlappedPartOfLeftRegionAtFront See getter for details
		 * @param nonOverlappedPartOfLeftRegionAtBack  See getter for details
		 */
		...
		/**
		 * @return any part of the left master region that does not overlap at the back,
		 *         or null if there is no part of the left master region ending behind
		 *         the right region.
		 */
		...
		/**
		 * @return any part of the left master region that does not overlap at the
		 *         front, or null if there is no part of the left master region starting
		 *         before the right region.
		 */
		...
		/**
		 * @return a {@link MediumRegion} that is the overlapping part of the two
		 *         {@link MediumRegion}s, containing the bytes in the overlap area taken
		 *         from the original left master {@link MediumRegion}.
		 */
	...
	/**
	 * Summarizes all relevant cases for types of overlaps between two
	 * {@link MediumRegion}s located on the same {@link Medium}.
	 */
	public enum MediumRegionOverlapType {
		/**
		 * The regions do not overlap with each other, e.g. the left region starts and
		 * ends before the right region.
		 */
		NO_OVERLAP,
		/**
		...
		SAME_RANGE,
		/**
		 * The left region is contained inside the right region. This does not include
		 * regions having {@link MediumRegionOverlapType#SAME_RANGE}, but it covers also
		 * the cases that both regions start or end at the same offset and the right
		 * region is bigger than the left on, i.e. cases where also
		 * {@link MediumRegion#overlapsOtherRegionAtFront(MediumRegion)} would return
		 * true. In the case that the left {@link MediumRegion} starts or ends at the
		 * same offset as the right {@link MediumRegion},
		 * {@link MediumRegion#determineRegionOverlap(MediumRegion, MediumRegion)}
		 * returns {@link #LEFT_FULLY_INSIDE_RIGHT}, and not
		 * {@link #LEFT_OVERLAPS_RIGHT_AT_FRONT} or
		 * {@link #LEFT_OVERLAPS_RIGHT_AT_BACK}.
		 */
		LEFT_FULLY_INSIDE_RIGHT,
		/**
		 * The right region is contained inside the left region. This does not include
		 * regions having {@link MediumRegionOverlapType#SAME_RANGE}, but it covers also
		 * the cases that both regions start or end at the same offset and the left
		 * region is bigger than the right on, i.e. cases where also
		 * {@link MediumRegion#overlapsOtherRegionAtFront(MediumRegion)} would return
		 * true.
		 */
		RIGHT_FULLY_INSIDE_LEFT,
		/**
		 * Represents the case that the left region overlaps the right region at its
		 * front, but it neither has the same range, nor is it fully inside the right
		 * region, nor does it enclose the right region.
		 * {@link #LEFT_OVERLAPS_RIGHT_AT_FRONT} represents a real overlap at front,
		 * i.e. the left region starts early as the right region, overlaps it at its
		 * front by at least one byte and ends earlier as the right region.
		 */
		LEFT_OVERLAPS_RIGHT_AT_FRONT,
		/**
		 * Represents the case that the left region overlaps the right region at its
		 * back, but it neither has the same range, nor is it fully inside the right
		 * region, nor does it enclose the right region.
		 * {@link #LEFT_OVERLAPS_RIGHT_AT_BACK} represents a real overlap at back, i.e.
		 * the left region starts after the right region, overlaps it at its back by at
		 * least one byte and ends after the right region.
		 */
		LEFT_OVERLAPS_RIGHT_AT_BACK,
	}

	/**
	 * Performs clipping of two {@link MediumRegion}s overlapping each other, where
	 * the left {@link MediumRegion} is considered as "master region", which means
	 * that the overlapped portion in the returned {@link MediumRegionClipResult}
	 * contains the bytes of the left {@link MediumRegion}. Only non-overlapping
	 * parts of the left region are returned. That said, the left
	 * {@link MediumRegion} is what counts here: Parts of the right
	 * {@link MediumRegion} never appear in the {@link MediumRegionClipResult}.
	 * 
	 * @param leftMasterRegion The left master {@link MediumRegion} to clip against
	 *                         the right region
	 * @param rightRegion      The right {@link MediumRegion} to clip against, must
	 *                         overlap the left region, must point to the same
	 *                         {@link Medium} as the left {@link MediumRegion}
	 * @return A {@link MediumRegionClipResult} containing all clipped portions of
	 *         the left master {@link MediumRegion}, see
	 *         {@link MediumRegionClipResult} for details.
	 */
	...
	/**
	 * Allows for readable code that uses the overlapping relations between two
	 * {@link MediumRegion}s. Two regions can in principle have all the overlap
	 * behaviors as described in the class {@link MediumRegionOverlapType}.
	 * 
	 * @param left  The left {@link MediumRegion}, must refer to the same
	 *              {@link Medium} as the right {@link MediumRegion}
	 * @param right The right {@link MediumRegion}, must refer to the same
	 *              {@link Medium} as the left {@link MediumRegion}, must point to
	 *              the same {@link Medium} as the left {@link MediumRegion}
	 ...
	 * 
	 * @param startReference The start {@link MediumOffset} of the
	 *                       {@link MediumRegion}.
	 * @param buffer         The bytes buffered in this {@link MediumRegion}. May be
	 *                       empty. The remaining bytes are considered as being
	 *                       cached.
	 */
	...
	/**
	 * Creates a new {@link MediumRegion} not backed by cached bytes, but simply
	 * representing a region on the external medium.
	 * 
	 * @param startReference The start {@link MediumOffset} of the
	 *                       {@link MediumRegion}.
	 * @param size           The size of the region in bytes. May be 0 to represent
	 *                       the empty region.
	 */
	...
	/**
	 * Calculates and returns the {@link MediumOffset} pointing to the first byte
	 * after this {@link MediumRegion}.
	 * 
	 * @return the {@link MediumOffset} pointing to the first byte after this
	 *         {@link MediumRegion}.
	 */
	...
	/**
	 * Calculates and returns the long offset pointing to the first byte after this
	 * {@link MediumRegion}. This method is slightly more efficient as it does not
	 * create a new {@link MediumOffset} (that probably is maintained in a pool).
	 * 
	...
	/**
	 * Returns whether the given {@link MediumOffset} is contained in the
	 * {@link MediumRegion}, i.e. is between {@link #getStartOffset()} inclusive and
	 * {@link #calculateEndOffset()} exclusive.
	 * 
	 * @param reference The {@link MediumOffset} to test for being contained.
	...
	/**
	 * Returns the bytes cached in this {@link MediumRegion}, if any. If there are
	 * cached bytes, only the bytes remaining in the returned read-only
	 * {@link ByteBuffer} must be considered to be cached in this
	 * {@link MediumRegion}, other bytes, if any, should be ignored. If there are no
	 * cached bytes, this method returns null.
	 * 
	 * @return the bytes cached in this {@link MediumRegion} or null if there are no
	 *         cached bytes.
	 */
	...
	/**
	 * Returns the number of bytes present both in this {@link MediumRegion} and in
	 * another {@link MediumRegion}.
	 * 
	 * @param other The other {@link MediumRegion} checked for shared bytes. Must
	 *              refer to the same {@link Medium} as this {@link MediumRegion}.
	 * @return The number of shared bytes between the two {@link MediumRegion}s or 0
	 *         if there are none.
	 */
	...
	/**
	 * Returns the number of bytes this {@link MediumRegion} represents. Equals the
	 * number of cached bytes, if there are any.
	 * 
	...
	/**
	 * Tells whether this whole {@link MediumRegion} is cached or not. If it is
	 * cached, {@link #getBytes()} returns a non-null and non-empty
	 * {@link ByteBuffer}
	 * 
	...
	/**
	 * Tells whether this {@link MediumRegion} overlaps another {@link MediumRegion}
	 * at its back or not. This method returns true in the following cases (S stands
	 * for start byte, E for end byte, "_" stands for any other byte):
	 * <p>
	 ...
	 * <p>
	 * 
	 * That said, {@link #overlapsOtherRegionAtBack(MediumRegion)} and
	 * {@link #overlapsOtherRegionAtFront(MediumRegion)} only both return true in
	 * the case that this and the other region are equal.
	 * 
	 * @param other The other {@link MediumRegion} checked for overlaps by this
	 *              {@link MediumRegion}. Must refer to the same {@link Medium} as
	 *              this {@link MediumRegion}.
	...
	/**
	 * Tells whether this {@link MediumRegion} overlaps another {@link MediumRegion}
	 * at its front or not. This method returns true in the following cases (S
	 * stands for start byte, E for end byte, "_" stands for any other byte):
	 * <p>
	...
	public boolean overlapsOtherRegionAtFront(MediumRegion other) {
		...
		return otherStartRef.behindOrEqual(startRef) && (otherStartOffset < myEndOffset)
			&& (otherEndOffset >= myEndOffset);
	}

	/**
	 * Resets the internal {@link ByteBuffer} and ensures that the size of the
	 * region gets updated correctly.
	 * 
	...
	/**
	 * Splits this {@link MediumRegion} at the given {@link MediumOffset} and
	 * returns two new {@link MediumRegion} instances in any case. This existing
	 * {@link MediumRegion} instance is kept unchanged.
	 * 
	 * @param at The {@link MediumOffset} to split this {@link MediumRegion}. Must
	 *           be behind the start offset of this region, and must be smaller than
	 *           the end offset of this {@link MediumRegion}.
	 * 
	...
	public MediumRegion[] split(MediumOffset at) {
		...
		if (isCached()) {
			ByteBuffer firstRegionBuffer = getBytes();
			firstRegionBuffer.limit(firstRegionBuffer.position() + firstRegionSize);

			ByteBuffer secondRegionBuffer = getBytes();
	...
	/**
	 * Returns an offset interval string representation of this
	 * {@link MediumRegion}, omitting any {@link Medium} information.
	 */
