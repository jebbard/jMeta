...
/**
 * {@link MediumDataProvider} is a utility class providing some convenience
 * method for controlled access to the medium via a {@link MediumStore}. The
 * main purpose is to ensure that cache fragmentation is low, especially by
 * implementing the method {@link #bufferBeforeRead(MediumOffset, long)} which
 * should be called before actually getting data using
...
public class MediumDataProvider {
	 ...
	 *
	 * @param mediumStore The {@link MediumStore} to use, must not be null
	 */
	...
	/**
	 * Tries to buffer data starting at the given start {@link MediumOffset} by
	 * using the medium caching provided by {@link MediumStore}. The main purpose of
	 * this method is to ensure a sensible buffering that does not lead to too much
	 * cache fragmentation.
	 *
	 * The given size is just used as rough indicator as follows:
	 * <ul>
	 * <li>size is allowed to be specified as
	 * {@link DataBlockDescription#UNDEFINED}. If so, caching only happens if there
	 * are currently exactly 0 bytes cached at the given start {@link MediumOffset}.
	 * In that case, exactly {@link Medium#getMaxReadWriteBlockSizeInBytes()} are
	 * cached.</li>
	 * <li>Otherwise if size is bigger than
	 * {@link Medium#getMaxReadWriteBlockSizeInBytes()}, then only exactly
	 * {@link Medium#getMaxReadWriteBlockSizeInBytes()} are cached just once.</li>
	 * <li>Otherwise if less than size bytes are currently cached at the given
	 * offset, this method caches {@link Medium#getMaxReadWriteBlockSizeInBytes()}
	 * starting from the first offset that is not already cached</li>
	 * </ul>
	 *
	 * In case of reaching the end of medium during caching, this incident is just
	 * logged and otherwise ignored.
	 *
	 * @param startOffset The start {@link MediumOffset} for starting buffering
	 * @param size        The size indicator for buffering, might be
	 *                    {@link DataBlockDescription#UNDEFINED} to buffer
	 *                    {@link Medium#getMaxReadWriteBlockSizeInBytes()} bytes if
	 *                    none are buffered at the offset yet
	 */
	public void bufferBeforeRead(MediumOffset startOffset, long size) {
		...
		if (cacheOffset != null) {
			try {
				...
				// This is not necessarily an error condition, and for buffering it is safe to
				// ignore this
				MediumDataProvider.LOGGER.debug("Reached end of medium during buffering", e);
			}
		}
	}

	/**
	 * Creates a new instance of a {@link FieldDataProvider} able to provide
	 * subsequent byte chunks from the medium starting at the given start offset.
	 *
	 * @param startOffset The start {@link MediumOffset}
	...
	/**
	 * Retrieves data from the medium starting at the given offset with the given
	 * size. Assumes that the number of bytes is actually present and throws a
	 * runtime exception in case of an end of medium encountered.
	 *
	 * See {@link MediumStore#getData(MediumOffset, int)} for more details.
	 *
	 * @param startOffset The start offset
	 * @param size        The number of bytes to fetch
	 * @return A {@link ByteBuffer} according to
	 *         {@link MediumStore#getData(MediumOffset, int)}
	 */
