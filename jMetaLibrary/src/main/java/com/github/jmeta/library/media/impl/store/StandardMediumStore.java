...
/**
 * {@link StandardMediumStore} is the default implementation of the
 * {@link MediumStore} interface.
 */
public class StandardMediumStore<T extends Medium<?>> implements MediumStore {
	 ...
	 *
	 * @param mediumAccessor The {@link MediumAccessor} instance to use, also
	 *                       contains the {@link Medium} this {@link MediumStore}
	 *                       works on
	 * @param cache          The {@link MediumCache} instance to use
	 * @param offsetFactory  The {@link MediumOffsetFactory} instance to use
	 * @param changeManager  The {@link MediumChangeManager} to use
	 */
	...
	/**
	 * In case of {@link MediumRegion}s overlapping just front or back of a given
	 * range, they need to be clipped, which is done by this method. It returns a
	 * clipped region that is guaranteed to start at or behind the range start
	 * offset and end at or before the range end offset.
	 *
	 * @param region      The {@link MediumRegion} to clip
	 * @param rangeOffset The start offset of the range
	...
	/**
	 * Ensures that the underlying {@link Medium} is opened for writing and
	 * writable.
	 */
	...
	public void flush() {
		...
		logDebugMessage(
			() -> "STARTING Flush, having " + changeManager.getScheduledActionCount() + " scheduled changes");
		...
		for (MediumAction scheduledAction : scheduledActions) {
			...
			case INSERT:
				...
				// If there is an existing cached region containing the insert offset, we must
				// split it there,
				// to ensure the part of the region behind the insert offset is shifted
				// correspondingly to leave room
				// for the inserts
	...
	public ByteBuffer getData(MediumOffset offset, int numberOfBytes) throws EndOfMediumException {
		...
		// This "if" is just an optimization for the 80% case in which we just take the
		// original ByteBuffer as view,
		// tailored to the requested range. This safes us new memory allocation and
		// copying of bytes.
	...
	/**
	 * Determines if a given cached {@link MediumRegion}, that was still cached at
	 * point in time {@link MediumCache#getRegionsInRange(MediumOffset, int)} was
	 * meanwhile has become uncached as new regions have been added after this
	 * initial call which forced the cache to remove previously cached
	 * {@link MediumRegion}s to keep its size below the maximum allowed cache size.
	 *
	 * If the region is now uncached, it can nevertheless be used for caching again
	 * or retrieving data. This way we also avoid unneeded medium accesses even in
	 * this corner case.
	 *
	 * @param cachedRegion            The cached {@link MediumRegion} to check
	 * @param initialCacheSizeInBytes The cache size at point in time when
	 *                                {@link MediumCache#getRegionsInRange(MediumOffset, int)}
	 *                                was called
	 * @param maxNumberOfBytesToAdd   The maximum number of bytes that might get
	 *                                added to the cache in total
	 *
	 * @return true if the given {@link MediumRegion} has now become uncached, false
	 *         otherwise
	 */
	...
	private void logDebugMessage(Supplier<String> message) {
		if (StandardMediumStore.LOGGER.isDebugEnabled()) {
			StandardMediumStore.LOGGER.debug(message.get());
		}
	}
	...
	/**
	 * Only for non-random access media: If the medium's current position is smaller
	 * than the given offset, it reads all bytes between these offsets chunk-wise
	 * and returns the corresponding {@link MediumRegion}s. If the medium's current
	 * position is bigger than the given offset, it throws an
	 * {@link InvalidMediumOffsetException}. If the offsets are equal, it returns an
	 * empty list. For random-access media it also returns an empty list.
	 *
	 * @param offset The target offset
	 * @return The {@link MediumRegion}s of maximum size
	 *         {@link Medium#getMaxReadWriteBlockSizeInBytes()} between the current
	 *         position and the given offset, if any
	 * @throws EndOfMediumException if the end of the medium is reached during
	 *                              reading
	 */
		...
		if (!getMedium().isRandomAccess()) {
			...
			if (offset.before(currentPosition)) {
				logDebugMessage(() -> "Checking cache data from offset " + currentPosition.getAbsoluteMediumOffset()
					+ " until the current position" + offset.getAbsoluteMediumOffset()
					+ " for non-random-access medium");
	 ...
	 * @throws EndOfMediumException in case of EOM was reached during reading
	 */
	...
	/**
	 * Determines chunked {@link MediumRegion}s filled with data read from the
	 * medium in the given range based on the maximum read-write block size
	 * configured for the medium.
	 *
	 * @param rangeOffset The range start offset
	 * @param rangeSize   The range's size in bytes
	 * @return The list of all {@link MediumRegion}s of maximum size
	 *         {@link Medium#getMaxReadWriteBlockSizeInBytes()} covering the whole
	 *         range.
	 * @throws EndOfMediumException if the end of the medium is reached during
	 *                              reading
	 */
	private List<MediumRegion> readRegionWise(MediumOffset rangeOffset, long rangeSize) throws EndOfMediumException {
		...
		regionsRead = MediumRangeChunkAction.performActionOnChunksInRange(MediumRegion.class,
			EndOfMediumException.class, rangeOffset, rangeSize, getMedium().getMaxReadWriteBlockSizeInBytes(),
			this::readRegion);
		return regionsRead;
	}
