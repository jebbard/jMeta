...
/**
 * {@link AbstractWritableRandomAccessMediumAccessorTest} is the base class for
 * testing {@link MediumAccessor} instances that are random-access media and
 * thus can also be modified by writing. All {@link MediumAccessor} instances
 * returned by {@link #createImplementationToTest()} must be thus enabled for
 * writing.
 *
 * This class contains all test cases specific to those {@link MediumAccessor}
 * instances, specifically the tests of
...
public abstract class AbstractWritableRandomAccessMediumAccessorTest extends AbstractMediumAccessorTest {
	...
	/**
	 * Checks whether of the given {@link ByteBuffer} as written before can be
	 * re-read again with exactly the same content.
	 * 
	 * @param mediumAccessor The implementation to test
	 * @param writeReference The {@link StandardMediumOffset} where the
	 *                       {@link ByteBuffer} has been written to.
	 * @param dataWritten    The {@link ByteBuffer} written before.
	 */
	...
	/**
	 * Executes a call to {@link MediumAccessor#write(ByteBuffer)} with hard-coded
	 * dummy bytes and returns the dummy bytes written.
	 * 
	 * It already checks the content of the byte buffer written to have the correct
	 * remaining bytes-
	 * 
	 * @param mediumAccessor    The {@link MediumAccessor} to test
	 * @param writeReference    The {@link MediumOffset} to write to
	 * @param useReadOnlyBuffer true to use a read-only buffer when writing, false
	 *                          otherwise
	...
	/**
	 * Checks whether the bytes between two {@link MediumOffset} instances is
	 * unchanged after a write operation.
	 * 
	 * @param mediumAccessor      The implementation to test
	 * @param rangeStartReference The {@link MediumOffset} pointing to the start of
	 *                            the range to check.
	 * @param rangeEndReference   The {@link MediumOffset} pointing to the end of
	 *                            the range to check, i.e. the first offset not
	 *                            checked.
	 */
	...
	/**
	 * Tests the {@link MediumAccessor#truncate()} method on the given
	 * {@link MediumAccessor} and with the given truncate {@link MediumOffset}.
	 * 
	 * @param mediumAccessor    The {@link MediumAccessor} to test
	 * @param truncateReference The truncate {@link MediumOffset}
	 * @param expectedNewLength The expected new {@link Medium} length
	 */
