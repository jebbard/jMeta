...
/**
 * {@link AbstractMediumStoreTest} is the base class for testing the
 * {@link MediumStore} interface. In contrast to
 * {@link AbstractReadOnlyMediumStoreTest}, it uses also writable media, and it
 * does not contain the default test cases for {@link MediumStore#open()},
 * {@link MediumStore#close()}, {@link MediumStore#getMedium()} and
 * {@link MediumStore#createMediumOffset(long)}, these can be found in
 * {@link AbstractReadOnlyMediumStoreTest}.
 *
 * Each subclass corresponds to a specific {@link Medium} type, a read-only or
 * writable {@link Medium} instance, as well as a random-access or
 * non-random-access medium. The sub-class hierarchy is as follows:
 * <ul>
 * <li>{@link AbstractMediumStoreTest}: Contains all tests that should run for
 * any medium type and access combinations, thus e.g. all general negative tests
 * (closed media and wrong reference)</li>
 * <li>{@link AbstractWritableRandomAccessMediumStoreTest}: Tests based on a
 * writable random-access medium; so all write test cases go here</li>
 * </ul>
 *
 * The filled media used for testing all must contain
 * {@link TestMedia#FIRST_TEST_FILE_CONTENT}, a string fully containing only
 * human-readable US-ASCII characters, and must be UTF-8 encoded. This
 * guarantees that 1 byte = 1 character. Furthermore, all bytes inserted must
 * also be human-readable US-ASCII characters with this property.
 *
 * There are specific naming conventions for testing
 * {@link MediumStore#getData(MediumOffset, int)} and
 * {@link MediumStore#cache(MediumOffset, int)}: [method name]_[medium
 * type]_[parameter values, esp. offset range]_[expected behaviour].
 *
 * @param <T> The type of {@link Medium} to use
 */
...
public abstract class AbstractMediumStoreTest<T extends Medium<?>> {
	...
	public void cache_forClosedMedium_anyRange_throwsException() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 10), 10);
	}
	...
	public void cache_forEmptyMedium_fromStart_throwsEOMException() {
		...
		testCache_throwsEndOfMediumException(TestMedia.at(currentMedium, 0), 1, currentMediumContent);
	...
	public void cache_forFilledMediumWithBigCache_cacheWithinPreviousCache_doesNotRead() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 2), 10);
	...
	public void cache_forFilledMediumWithBigCache_fromStartTillBeyondEOM_throwsEOMException() {
		...
		MediumOffset cacheOffset = TestMedia.at(currentMedium, 59);
	...
	public void cache_forFilledMediumWithBigCache_partlyCacheBeforeWithGaps_updatesCacheInGaps() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 10), 10);
		cacheNoEOMExpected(TestMedia.at(currentMedium, 30), 100);
		cacheNoEOMExpected(TestMedia.at(currentMedium, 135), 200);
	...
	public void cache_forFilledMediumWithSmallCache_cacheWithinPreviousCache_doesNotRead() {
		...
		verifyExactlyNReads(cacheSize / AbstractMediumStoreTest.MAX_READ_WRITE_BLOCK_SIZE_FOR_SMALL_CACHE);
	...
	public void cache_forFilledMediumWithSmallCache_fromStartAndMoreThanMaxCSize_cachesOnlyUpToMaxCSize() {
		...
		assertRangeIsNotCached(TestMedia.at(currentMedium, 0),
			(int) expectedActualCacheStartOffset.getAbsoluteMediumOffset());
	...
	public void cache_forReferencePointingToWrongMedium_anyRange_throwsException() {
		...
		cacheNoEOMExpected(TestMedia.at(TestMedia.OTHER_MEDIUM, 10), 10);
	}
	...
	public void close_onMediumStoreWithCachedContent_freesResources() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 0), 10);
	...
	public void getCachedByteCountAt_forClosedMedium_throwsException() {
		...
		Assert.assertEquals(0, mediumStoreUnderTest.getCachedByteCountAt(TestMedia.at(currentMedium, 0)));
	}
	...
	public void getCachedByteCountAt_forFilledMediumWithBigCache_noPriorCache_returnsZero() {
		...
		Assert.assertEquals(0, mediumStoreUnderTest.getCachedByteCountAt(TestMedia.at(currentMedium, 10)));
	}
	...
	public void getCachedByteCountAt_forReferencePointingToWrongMedium_throwsException() {
		...
		mediumStoreUnderTest.getCachedByteCountAt(TestMedia.at(TestMedia.OTHER_MEDIUM, 10));
	}
	...
	public void getData_forClosedMedium_anyRange_throwsException() {
		...
		getDataNoEOMExpected(TestMedia.at(currentMedium, 10), 10);
	}
	...
	public void getData_forEmptyMedium_fromStart_throwsEOMException() {
		...
		testGetData_throwsEndOfMediumException(TestMedia.at(currentMedium, 0), 1,
	...
	public void getData_forFilledMedium_fromMiddleToBeyondEOMAndNoCacheBefore_throwsEOMException() {
		...
		testGetData_throwsEndOfMediumException(TestMedia.at(currentMedium, 15), currentMediumContent.length(),
			currentMedium.getMaxReadWriteBlockSizeInBytes(), currentMediumContent);
	}
	...
	public void getData_forFilledMedium_fullMediumCacheBefore_returnsExpectedData() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 0), currentMediumContent.length());

		long getDataStartOffset = 5;
		int getDataSize = 400;

		testGetData_returnsExpectedData(TestMedia.at(currentMedium, getDataStartOffset), getDataSize,
			currentMediumContent);
	}
	...
	public void getData_forFilledMediumWithSmallCache_cacheWithMultipleRegionsBeforeAndStartBehindFirstRegionStart_doesNotReReadAndReturnsExpectedData() {
		...
		int blocksRead = 10;
		MediumOffset cacheOffset = TestMedia.at(currentMedium, 0);
		int cacheSize = blocksRead * AbstractMediumStoreTest.MAX_READ_WRITE_BLOCK_SIZE_FOR_SMALL_CACHE;
		cacheNoEOMExpected(cacheOffset, cacheSize);
	...
	public void getData_forFilledMediumWithSmallCache_twiceInEnclosingRegion_doesNotReReadAndReturnsExpectedData() {
		...
		// Still only one read done
		verifyExactlyNReads(getDataSize / AbstractMediumStoreTest.MAX_READ_WRITE_BLOCK_SIZE_FOR_SMALL_CACHE);
	...
	public void isAtEndOfMedium_forClosedMedium_throwsException() {
		...
		mediumStoreUnderTest.isAtEndOfMedium(TestMedia.at(currentMedium, 10));
	}
	...
	public void isAtEndOfMedium_forFilledMediumAndCacheUntilEOM_returnsTrue() {
		...
		cacheNoEOMExpected(TestMedia.at(currentMedium, 0), mediumSizeInBytes);

		Assert.assertTrue(mediumStoreUnderTest.isAtEndOfMedium(TestMedia.at(currentMedium, mediumSizeInBytes)));
	}
	...
	public void isAtEndOfMedium_forFilledMediumBeforeEOM_returnsFalse() {
		...
		Assert.assertFalse(mediumStoreUnderTest.isAtEndOfMedium(TestMedia.at(currentMedium, endOfMediumOffset / 2)));
	}
	...
	public void isAtEndOfMedium_forReferencePointingToWrongMedium_throwsException() {
		...
		mediumStoreUnderTest.isAtEndOfMedium(TestMedia.at(TestMedia.OTHER_MEDIUM, 10));
	}
	 ...
	 * {@link EndOfMediumException} as expected.
	 *
	 * @param cacheOffset          The offset to start caching
	 * @param cacheSize            The size to cache
	 * @param currentMediumContent The current content of the medium
	 */
	...
	/**
	 * Tests {@link MediumStore#getData(MediumOffset, int)} by comparing its result
	 * with the expected medium content.
	 *
	 * @param getDataOffset        The offset to use for the method call
	 * @param getDataSize          The size to use for the method call
	 * @param currentMediumContent The current medium content used to get the
	 *                             expected data
	 */
	...
	/**
	 * Tests {@link MediumStore#getData(MediumOffset, int)} to throw an end of
	 * medium exception when reaching it.
	 *
	 * @param getDataOffset        The offset to use for the method call
	 * @param getDataSize          The size to use for the method call
	 * @param chunkSizeToUse       The size of the read-write block chunks
	 * @param currentMediumContent The current content of the medium
	 */
	protected void testGetData_throwsEndOfMediumException(MediumOffset getDataOffset, int getDataSize,
		...
		try {
			...
			Assert.assertEquals(getDataSize < chunkSizeToUse ? getDataSize : chunkSizeToUse,
	...
	protected void verifyExactlyNReads(int N) {
		try {
			Mockito.verify(mediumAccessorSpy, Mockito.times(N)).read(ArgumentMatchers.anyInt());
		} catch (EndOfMediumException e) {
			throw new RuntimeException("Unexpected end of medium", e);
		}
	}
	...
	/**
	 * Verifies that there were exactly N calls to
	 * {@link MediumAccessor#write(ByteBuffer)} no matter which parameters used.
	 *
	 * @param N The number of expected calls
	 */
	protected void verifyExactlyNWrites(int N) {
		Mockito.verify(mediumAccessorSpy, Mockito.times(N)).write(ArgumentMatchers.any());
	}
}
