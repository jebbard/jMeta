...
/**
 * {@link WriteActionSequence} represents a sequence of
 * {@link MediumActionType#WRITE} operations as created by
 * {@link MediumChangeManager#createFlushPlan(int, long)} for insertion bytes
...
public class WriteActionSequence extends ExpectedActionSequence {
	 ...
	 * 
	 * @param startRef             The start offset for the expected write actions
	 * @param blockCount           The number of expected write actions
	 * @param blockSizeInBytes     The size of a block to write in bytes
	 * @param expectedBytes        The bytes expected to be contained in the write
	 *                             actions
	 * @param byteBufferStartIndex An offset in the byte buffer where the comparison
	 *                             (and dumping) should start
	 */
	...
	public void assertFollowsSequence(Iterator<MediumAction> actionIter) {
		...
		for (int i = 0; i < getBlockCount(); i++) {
			ExpectedActionSequence.expectWriteAction(actionIter, nextExpectedWriteRef, getBlockSizeInBytes(),
	...
	public void dump(PrintStream stream) {
		...
		for (int i = 0; i < getBlockCount(); i++) {
			ExpectedActionSequence.dumpMediumAction(stream,
				new MediumAction(MediumActionType.WRITE, new MediumRegion(nextExpectedWriteRef, getBlockSizeInBytes()),
					0, ByteBuffer.wrap(expectedBytes.array(), byteBufferStartIndex + (i * getBlockSizeInBytes()),
						getBlockSizeInBytes())));
			nextExpectedWriteRef = nextExpectedWriteRef.advance(getBlockSizeInBytes());
		}
	}
