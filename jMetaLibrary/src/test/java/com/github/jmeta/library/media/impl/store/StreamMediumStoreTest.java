...
/**
 * {@link StreamMediumStoreTest} tests a {@link MediumStore} backed by
 * {@link InputStreamMedium} instances with a cache.
 */
public class StreamMediumStoreTest extends AbstractMediumStoreTest<InputStreamMedium> {
	...
	public void cache_forFilledStreamMediumWithBigCache_fromMiddle_cachesUpToStartOffset() {
		...
		MediumOffset mediumStartOffset = TestMedia.at(currentMedium, 0);
	...
	public void cache_forFilledStreamMediumWithBigCache_multipleOverlappingAndDisconnectedRegions_cachesExpectedBytes() {
		...
		assertRangeIsCachedFromExternalMedium(TestMedia.at(currentMedium, 0),
	...
	public void cache_forFilledStreamMediumWithSmallCache_middleOffset_readsBlockWiseUpToStartOffsetAndCachesOnlyLastRegions() {
		...
		// Cache more bytes than max cache size such that regions cached at the
		// beginning of the medium need to be freed
		// automatically
		MediumOffset cacheOffset = TestMedia.at(currentMedium, 20);
		int expectedActualCacheSize = AbstractMediumStoreTest.MAX_CACHE_SIZE_FOR_SMALL_CACHE;
		int cacheSize = expectedActualCacheSize + 100;

		MediumOffset expectedActualCacheStartOffset = TestMedia.at(currentMedium, 120);

		cacheNoEOMExpected(cacheOffset, cacheSize);

		verifyExactlyNReads(((int) (cacheOffset.getAbsoluteMediumOffset() + cacheSize)
			/ AbstractMediumStoreTest.MAX_READ_WRITE_BLOCK_SIZE_FOR_SMALL_CACHE) + 1);
		...
		Assert.assertEquals(0, mediumStoreUnderTest.getCachedByteCountAt(TestMedia.at(currentMedium, 119)));
	...
	public void cache_forFilledStreamMediumWithSmallCache_offsetInPreviouslyFreedCacheRegion_throwsException() {
		...
		// beginning of the medium need to be freed
		// automatically
		cacheNoEOMExpected(TestMedia.at(currentMedium, 20),
			AbstractMediumStoreTest.MAX_CACHE_SIZE_FOR_SMALL_CACHE + 100);

		cacheNoEOMExpected(TestMedia.at(currentMedium, 10), 20);
	}
	...
	protected InputStreamMedium createEmptyMedium(String testMethodName) throws IOException {
		return new InputStreamMedium(new FileInputStream(TestMedia.EMPTY_TEST_FILE_PATH.toFile()),
			StreamMediumStoreTest.STREAM_BASED_EMPTY_MEDIUM_NAME);
	}
	...
	protected InputStreamMedium createFilledMedium(String testMethodName, long maxCacheSize, int maxReadWriteBlockSize)
		...
		return new InputStreamMedium(new FileInputStream(TestMedia.FIRST_TEST_FILE_PATH.toFile()),
			StreamMediumStoreTest.STREAM_BASED_FILLED_MEDIUM_NAME, maxCacheSize, maxReadWriteBlockSize);
	}
	...
	public void getData_forFilledMediumWithBigCache_rangeBeforeCachedRange_returnsExpectedDataAndDoesNotAccessMediumAgain() {
		...
		MediumOffset cacheOffset = TestMedia.at(currentMedium, 100);
		int cacheSize = 100;
	...
	public void getData_forFilledMediumWithBigCache_unCachedRange_returnsExpectedDataAccessesMediumTwiceAndUpdatesCache() {
		...
		MediumOffset getDataOffset = TestMedia.at(currentMedium, 15);
		int getDataSize = 80;
		...
		Assert.assertEquals(getDataSize + getDataOffset.getAbsoluteMediumOffset(),
			mediumStoreUnderTest.getCachedByteCountAt(TestMedia.at(currentMedium, 0)));
	...
	public void getData_forFilledStreamMediumWithBigCache_forOffsetBeyondEOM_throwsEOMException() {
		...
		try {
			mediumStoreUnderTest.getData(TestMedia.at(currentMedium, getDataStartOffset), getDataSize);
			Assert.fail("Expected " + EndOfMediumException.class);
		} catch (EndOfMediumException e) {
			Assert.assertEquals(TestMedia.at(currentMedium, 0), e.getReadStartReference());
		...
		assertRangeIsCachedFromExternalMedium(TestMedia.at(currentMedium, 0), currentMediumContent.length(),
			currentMediumContent);
	}
	...
	public void getData_forFilledStreamMediumWithSmallCache_offsetInPreviouslyFreedCacheRegion_throwsException() {
		...
		getDataNoEOMExpected(TestMedia.at(currentMedium, 10), 20);
	}
	...
	protected String getMediumContentAsString(InputStreamMedium medium) {
		if (medium.getName().equals(StreamMediumStoreTest.STREAM_BASED_EMPTY_MEDIUM_NAME)) {
			return TestMedia.EMPTY_TEST_FILE_CONTENT;
		} else if (medium.getName().equals(StreamMediumStoreTest.STREAM_BASED_FILLED_MEDIUM_NAME)) {
