...
public class MediumOffsetEqualityTest extends AbstractEqualsTest<MediumOffset> {
   ...
   private List<Medium<?>> createDifferentMedia() {
      ...
      media.add(new InMemoryMedium(new byte[] { 1 }, "Hallo4", MediumAccessType.READ_ONLY));
   ...
   private List<Medium<?>> createMedia() {
      ...
      media.add(new InMemoryMedium(MediumOffsetEqualityTest.BYTES, "Hallo4", MediumAccessType.READ_ONLY));
   ...
   private List<MediumOffset> createMediumOffsets(List<Medium<?>> media, long baseOffset) {
      ...
      for (int i = 0; i < 3; ++i) {
         mediumReferences.add(new StandardMediumOffset(
            new FileMedium(TestMedia.FIRST_TEST_FILE_PATH, MediumAccessType.READ_WRITE), baseOffset + i * 10));
      }
