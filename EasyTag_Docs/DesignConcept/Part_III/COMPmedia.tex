...
\begin{itemize}
	...
	\item \texttt{read} at the current position: Read $n$ bytes from external medium by explicit access from the current position, return the bytes in a \texttt{ByteBuffer}. For byte stream media, the timeout mechanism according to \DesLink{dd:426} is implemented here. Advances the current position by the number of read bytes.
