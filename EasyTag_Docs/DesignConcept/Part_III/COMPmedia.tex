...
\DD{dd:411d}
{% Titel
The \COMPmedia{} API does not provide the option to skip bytes of an \texttt{InputStream}s instead of reading them into the cache.
}
{% Kurzbeschreibung
It sounds like a good idea to offer the configurable possibility to skip bytes from an \texttt{InputStream} instead of unnecessarily read them (and put them into a cache). This mainly applies for the case when you are currently at offset $x$, but now you want to read bytes from the stream starting at offset $x+100$. You might never want to access the bytes between $x$ and $x+100$. So it would be worthwile to allow the possibility to simply skip them instead of reading them into memory and into the cache.
...
{% Begründung
It is too complex to implement the method reliably, thus we omit it and only offer the possibility to read ahead until a given offset. All bytes read are possibly read into the cache. However, the cache is ``self-cleaning'' if a maximum size is configured. This mechanism should be sufficient for achieving a moderate heap usage.
}
{% Nachteile
...
{% Begründung
We assume that reading data from files takes a while and take into account that it might block. Reading data from files is, however, usually not a candidate for long or even any blocking. Althout using interruptible channels, file I/O might be interruptible after timeouts, it is not implemented due to its added complexity without much added value.

For \texttt{InputStream}s, like media streams or socket connections, however, blocking is a quite possible case. Thus timeouts when reading from byte streams are sometimes useful and \LibName{} offers to configure them for the \texttt{read} operation.

Writing operations for byte streams are not supported according to \DesLink{dd:403}. Thus, in general, no timeouts are implemented for write operations.
}
{% Nachteile
...
\begin{itemize}
	...
	\item \texttt{Open}: Opens the medium for access. Opening creates an exclusive lock on the medium, as far as the medium supports this (see \DesLink{dd:404}, \DesLink{dd:405}, \DesLink{dd:406}). It is an implicit operation directly executed with the constructor call.
	...
	\item \texttt{Read} from offset $x$: Read $n$ bytes from external medium by explicit access, return the bytes in a \texttt{ByteBuffer}. For non-random-access media, it is only allowed to provide a constant as parameter that indicates ``next bytes, offset-independent''. For random-access media, an arbitrary valid offset $x$ can be passed. For byte stream media, the timeout mechanism according to \DesLink{dd:426} is implemented here.
	\item \texttt{Write} to offset $x$: Writes $n$ bytes to the external medium at a given offset $x$, passed in a \texttt{ByteBuffer}. Throws an exception for read-only media, especially for \texttt{InputStream}.
        \item \texttt{Truncate} to new end offset $x$: Truncates the medium to end at the new end offset $x$. Throws an exception for read-only media, especially for \texttt{InputStream}.
...
We have to also investigate a bit futher on read timeouts. In \DesLink{dd:426}, we decided to offer timeouts for the \texttt{read} operation. The question is: How is it done and what are the drawbacks?

TODO: Hier weiter - 
1. Designentscheidung, dass ein weiterer Thread nötig ist und ein Future genutzt wird, sowie dass partielle Daten zurückgeliefert werden.
2. Designentscheidung, dass versucht wird, den laufenden Thread zu ``interrupten'' + den Stream zu schließen und er dann erneut geöffnet wird (was zu Datenverlust führen kann)
3. Designentscheidung: Es muss explizit auf die Nachteile des read timeouts (Perf. Overhead, schwieriges Beenden der Threads auf manchen Plattformen) in der Doku hingewiesen werden + gute Alternativen (direkte Socket timeouts auf SocketInputStreams etc.)
