...
/**
 * {@link EnhancedArrays} provides further array algorithms not contained in {@link Arrays}.
 */
public class EnhancedArrays {

   /**
    * Merges a list of arrays in the order as given in the corresponding parameter. The total length of the resulting
    * merged array must be available before calling this method due to performance issues - it safes one iteration
    * through the whole list. When filling the list with the fragment arrays, the total length can be calculated.
    *
    * @param arrayList
    *           A list of arrays that are fragments and are to be merged to a resulting array.
    * @param totalLength
    *           The overall total length of the destination array.
    *
    * @return A merged array containing all array fragments in the given order.
    *
    * @throws IndexOutOfBoundsException
    *            If the given total length does not match the sum of all array fragment lengths.
    */
   ...
   /**
    * Determines the offset of the first occurrence of a sub-array within a given array of the same type.
    * 
    * @param array
    *           The array to search in. Its length must be equal or longer than the length of the array to find plus the
    *           given start offset.
    * @param subArray
    *           The array to find. Its length must be equal or shorter than the length of the array to search in minus
    *           the given start offset.
    * @param startOffset
    *           The offset in the array to start searching for the sub-array. Must not be negative. Must not be greater
    *           or equal to the array's length.
    * @param increase
    *           The count the offset for searching the sub-array in the given array is increased. E.g. if this parameter
    *           is 2, this method will find occurrences of the given sub-array at offsets 0, 2, 4, 6 .. It will not find
    *           occurrences at offsets 1, 3, 5, 7 .. This is useful if sub-arrays are required to start at word or dword
    *           offsets, e.g. for finding byte representations of multi-byte characters. Must not be smaller than 1.
    *
    * @return The offset of the first occurrence of toFind within searchIn or -1 if the array to find is not contained
   ...
   /**
    * Copies the specified range of the specified array into a new array. The initial index of the range (from) must lie
    * between zero and original.length, inclusive. The value at original[from] is placed into the initial element of the
    * copy (unless from == original.length or from == to). Values from subsequent elements in the original array are
    * placed into subsequent elements in the copy. The final index of the range (to), which must be greater than or
    * equal to from, may be greater than original.length, in which case (byte)0 is placed in all elements of the copy
    * whose index is greater than or equal to original.length - from. The length of the returned array will be to -
    * from.
    *
    * @param original
    ...
    *            {@literal if from < 0  or from > original.length()}
    */
   ...
   /**
    * Converts a {@link Collection} of the wrapper type {@link Byte} to an array of byte.
    *
    * @param byteCollection
    *           The list of {@link Byte}s to convert.
   ...
   /**
    * Parses a byte array from a string representation. The string representation is expected to have a format very
    * similar to a Java byte array. See the complex value of the regular expression pattern
    * {@link #ARRAY_AS_STRING_PATTERN}.
    *
    * The pattern allows for specification of a multiplicity using the '*' symbol. If the specified multiplicity times
    * the number of array elements in the string exceeds {@link Integer#MAX_VALUE}, the
    *
    ...
    *            If the given array string representation does not match the regular expression @link
   ...
   public static byte[] parseArray(String arrayAsString) throws InvalidArrayStringFormatException {
      ...
      Matcher matcher = ARRAY_AS_STRING_PATTERN.matcher(arrayAsString);

      if (!matcher.matches())
         throw new InvalidArrayStringFormatException(
            "Array string must " + "match regular expression <  " + ARRAY_AS_STRING_PATTERN.pattern() + "  >.");
   ...
   /**
    * The pattern to match for a array as string expression as required by {@link #parseArray(String)}.
    */
