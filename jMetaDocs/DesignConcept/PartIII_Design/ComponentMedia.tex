...
\item \emph{replace} $N$ bytes at a given offset by $M$ new bytes (adresses requirement \texttt{AMed03}). Thus, replace has two flavors:
\begin{enumerate}
...
This leaves open \texttt{AMed04} and \texttt{AMed05} which we will adress later when discussing the details of the two-stage write protocol.
...
{% Kurzbeschreibung
\begin{enumerate}
\item Step 1: \emph{insert} at offset $x$, Step 2: \emph{insert} at different offset: Do not influence each other
...
{% Nachteile
You could alternatively interpret ``insert'' as such that the second call inserts data \emph{before} the previous earlier one, which might lead to surprises in a few use scenarios.
}
%%%% <-- DD %%%%
...
\DD{dd:422b}
{% Titel
\emph{insert} and \emph{remove} do not interact with each other in any way
}
{% Kurzbeschreibung
The following cases can be identified:
\begin{enumerate}
\item Step 1: \emph{insert} at offset $x$, Step 2: \emph{remove} at same offset: Do not influence each other
\item Step 1: \emph{remove} at offset $x$, Step 2: \emph{insert} at same offset: Do not influence each other
\item Step 1: \emph{remove} $n=10$ bytes at offset $x$, Step 2: \emph{insert} at offset $x+5$: Do not influence each other
\item Step 1: \emph{insert} at offset $x+5$, Step 2: \emph{remove} $n=10$ bytes at offset $x$: Do not influence each other
\end{enumerate}
Thus, \emph{insert} and \emph{remove} do not at all influence each other, no matter in which order the calls are done and if their offsets or ranges overlap or not. Thus, especially, it is not possible to \emph{remove} content that was earlier inserted.
}
{% Begründung
The formal reason is based on \DesLink{dd:421}. As \emph{remove} never can refer to offsets that are not yet persisted on the medium, it cannot refer to data pending to be inserted. So how can we interpret the overlap case? If you remove a range on the external medium, and (afterwards or before) insert new bytes in the middle of the range, this means: The inserted bytes will be inserted after the first byte located before the beginning of the removed range. You can imagine the inserted bytes ``falling'' back to the offset where the removal starts.
}
{% Nachteile
...
Now we are left with clarifying how \emph{insert} interacts with \emph{replace}.

%%%% DD --> %%%%
\DD{dd:422d}
{% Titel
\emph{insert}s within a replaced range are forbidden, \emph{insert}s within a range practically removed by a replace action are still allowed
}
{% Kurzbeschreibung
First of all, it is clear that if the insertion offset is completely outside of the replaced range and the range of the replacement bytes, there are no interactions. Now we have to consider the following special cases:
\begin{enumerate}
\item Step 1: \emph{insert} $n_{1}$ bytes at offset $x$, Step 2: \emph{replace} $m$ bytes by $n_{2}$ new bytes at same offset: This must behave the same way as multiple inserts, actually concatenating the data in call order. The final result after flush: 
\begin{itemize}
\item At offset $x$, the insertion data of the first operation (insert)
\item At offset $x+n_{1}$, the replacement data of the second operation (replace)
\end{itemize}
\item Step 1: \emph{replace} $m$ bytes by $n_{1}$ new bytes at offset $x$, Step 2: \emph{insert} $n_{2}$ new bytes at same offset: Like in the previous case, the final result after flush is: 
\begin{itemize}
\item At offset $x$, the replacement data of the first operation (replace)
\item At offset $x+n_{2}$, the inserted data of the second operation (insert)
\end{itemize}
\item Step 1: \emph{replace} $m$ bytes by $n_{1}<m$ new bytes at offset $x$, Step 2: \emph{insert} new bytes at offset $x+n_{1}+1$: For a removing \emph{replace}, it is allowed to insert new bytes in the removed range
\item Step 1: \emph{insert} new bytes at offset $x+n_{1}+1$, Step 2: \emph{replace} $m$ bytes by $n_{1}<m$ new bytes at offset $x$: For a removing \emph{replace}, it is allowed to insert new bytes in the removed range
\item Step 1: \emph{insert} at offset $x+5$, Step 2: \emph{replace} of $n=10$ bytes at offset $x$, especially for inserting replaces: Is not allowed and leads to a runtime exception
\item Step 1: \emph{replace} $n=10$ bytes at offset $x$, Step 2: \emph{insert} at offset $x+5$: Same as previous case
\end{enumerate}
}
{% Begründung
For (1) and (2): This way, the user can do replacements and insertion as if linearly adding stuff to e.g. a tag. E.g. he can first replace the content of an existing field, then insert a new field, or the other way round. The result will reflect the order of invocations.
For (3) and (4): This is the same interaction as between insert and remove.
For (5) and (6): How to make sense of ``insert bytes in the middle of a range that has to be replaced with new bytes''? Of course we could say, exactly this is what happens. But it turned out that complex logic is necessary to mix insertion bytes into replacement bytes. The user can achieve the same by simply including the insertion bytes in the replacement bytes starting at offset $x+5$.
}
{% Nachteile
For (5) and (6): This does not make things possible like: The user first replaces a range of bytes, then wants to additionally insert something within the block of replacement bytes. Or the user inserts a new child field  into a parent tag, and then replaces the entire tag by something else.
}

Saying all this, it is not possible to implement \texttt{AMed04} (``it is possible to change or remove parts of an already done insertion before a flush'') and  \texttt{AMed05} (``it is possible to change or remove parts of an already done replacement before a flush'') by using subsequent inserts, removes and replaces. Then how is this requirement met? An example of this case is: We insert a new ID3v2 tag which already lead to scheduling an \emph{insert} with the corresponding bytes. Then afterwards and before a flush, the using code needs to change some flag values in the header of the ID3v2 tag. This must also change the inserted bytes of course. Here is how this could be done:
...
{% Kurzbeschreibung
\texttt{AMed04} can only be met outside \COMPmedia{}. Using code must detect that a parent of the changed field was already scheduled for insertion or replacment. If that is the case, they have to ``re-compute'' the overall insertion or replacment bytes, undo the previous insertion or replacement using \COMPmedia{} and finally schedule a new insertion or replacement containing the changed bytes.
}
{% Begründung
\COMPmedia{} could somehow support here by offering modification functions for already done \emph{insert}s or \emph{replace}s. However, this would also at least require the using code to detect the change in a parent, and also recompute. It would make the API of \MediumAction{} more complex and it would refuse to be a mostly constant class. Using \emph{undo}, the using code has just slightly more work to do.
}
{% Nachteile
No disadvantages known
}

We already clarified how insert interacts with remove and replace, in any other. We are now only left to clarify how multiple removes and replaces interact with each other. The operation \emph{remove} behaves as follows:
...
\item Step 1: \emph{remove} $n$ bytes at offset $x$, Step 2: \emph{remove} or \emph{replace} $m\geq n$ bytes at offset $y\leq x$ with $y+m\geq x+n$: If a call to \emph{remove} or \emph{replace} fully encloses the removed range of a previous \emph{remove} call, such a previous call is considered obsolete and is not processed during a \emph{flush}.
...
{% Begründung
There are no accidental multiple removals or replaces. For (2): It may happen that the user first removes a chield field, and additionally still before the flush \texttt{flush}, he removes the parent block. For (3) to (5): Would we allow the second calls, we would need to change the remove areas of the first calls. This leads to a more complex logic, that is not necessary according to the requirements stated in \SectionLink{sec:AnforderungenandaszweistufigeSchreibprotokoll}. A very important reason to not allow this is implementation of \texttt{undo}: Would we allow later changes of already made calls, then \texttt{undo} would be quite complex, because it would need to also undo such changes to removed or replaced regions, what basically means keeping a history of all changes made for the same region.
}
{% Nachteile
No disadvantages known
}
...
\DD{dd:424a}
{% Titel
\emph{replace} does not allow overlaps, only later calls with bigger regions make earlier calls obsolete
}
{% Kurzbeschreibung
The behaviour of \emph{replace} matches the behavior of \emph{remove} in terms of earlier calls. That means: If the replacement region of a \emph{replace} fully contains the replacement region of a previous call to \emph{replace}, this previous call is considered obsolete and not executed during \emph{flush}. Second, if the replacement region of a call is fully contained in the replacement region of a previous call, this is rejected with an exception. Third, whenever the replacement region of a call overlaps the replacement region of any previous call to \emph{replace} at front or back, the new call is rejected with an exception.
}
{% Begründung
See \DesLink{dd:424}
}
...
\item Is specific handling of \texttt{TRUNCATE} necessary?
...
\DD{dd:439b}
{% Titel
\texttt{READ} operations in \texttt{flush} prefer cached data and update the cache if not already contained 
}
{% Kurzbeschreibung
...
{% Kurzbeschreibung
Whenever \texttt{flush} processes an \texttt{INSERT} or inserting \texttt{REPLACE} action coming from the flush plan, it adds all bytes to insert or replacement bytes from these actions to the cache.
}
{% Begründung
We want to use cached data as much as possible and avoid unnecessary medium accesses.
}
...
Very similar for \texttt{REMOVE} and removing \texttt{REPLACE}:
%%%% DD --> %%%%
\DD{dd:439d}
{% Titel
\texttt{REMOVE} and removing \texttt{REPLACE} operations in \texttt{flush} remove their ranges from the cache
}
{% Kurzbeschreibung
Whenever \texttt{flush} processes a \texttt{REMOVE} or removing \texttt{REPLACE} action coming from the flush plan, it removes all bytes to remove or replace from the cache. Of course, the replacement bytes of a \texttt{REPLACE} action are still newly added to the cache.
}
{% Begründung
...
We still have the topic to update any already created \IMediumReference{}s when processing \texttt{INSERT}s, \texttt{REMOVE}s and \texttt{REPLACE}s according to design decision \DesLink{dd:418b}. When do we need to perform these changes? This actually depends on the action type:
%%%% DD --> %%%%
\DD{dd:439e}
{% Titel
For \texttt{INSERT}s and inserting \texttt{REPLACE}s, \IMediumReference{}s are updated \emph{BEFORE} the cache is updated, for \texttt{REMOVE}s and removing \texttt{REPLACE}s they are updated \emph{AFTERWARDS}
}
{% Kurzbeschreibung
For \texttt{INSERT}s and inserting \texttt{REPLACE}s, any \IMediumReference{}s already created and maintained in the \MediumReferenceRepository{} are updated first, which essentially increases the offsets of any \IMediumReference{}s behind the insertion offset by the number of bytes inserted. Only after this, the data newly inserted or replaced can be added to the cache.

In contrast to this, when \texttt{REMOVE}s and removing \texttt{REPLACE}s are processed, first of all, the bytes to remove, if present in the cache, need to be removed from it. Only then, all \IMediumReference{}s behind the remove offset need to be decreased by the number of removed bytes (or fall back to remove offset, if within the removed range). 
}
{% Begründung
For \texttt{INSERT}s and inserting \texttt{REPLACE}s: If we add the inserted data to the cache before updating reference, the following would happen: Assume there are bytes already cached directly starting at the insertion offset. They would be superseded by the inserted data, i.e. thrown out of the cache. Then only all remaining data with higher offsets in the cache would be updated. This way, we would essentially lose cached data unnecessary and increase the complexity of cache management needed at runtime a bit.

For \texttt{REMOVE}s and removing \texttt{REPLACE}s: If we would first update the \IMediumReference{}s here, the following could happen: Assume we have two consecutive cached regions within the region to remove. For both regions, its offset would be updated to point to the remove offset. So we would have two regions cached with the same start offset, which leaves the cache inconsistent. Instead, we first remove all cached regions within the removed range, then we update any remaining regions (those behind the removed range).
}
{% Nachteile
No disadvantages known
}
...
\item Iterate all actions of the read-write-plan, the following steps are done for each action.
\item Execute the \MediumAction{} at the indicated offset, we distinguish the following cases:
\begin{itemize}
\item If action = \texttt{READ}: Read $n$ bytes \-- that are written in the subsequent action \-- where $n$ is smaller than the maximum write block size and bigger than 0. For that, first all regions are determined using \MediumCache{}\texttt{.getData()}. Those that are not cached are read by direct access to the medium via \IMediumAccessor{}. Then a corresponding \texttt{ByteBuffer} is built up step-wise. If an \texttt{EndOfMediumException} occurs, this is considered as impossible and an \texttt{IllegalStateException} is thrown.
\item If action = \texttt{WRITE}: If the \texttt{WRITE} action contains bytes already, it as associated to an \texttt{INSERT} or \texttt{REPLACE}. In this case, directly write the contained bytes with direct access to the medium via \IMediumAccessor{}. If the action does not contain any bytes, the previous action must have been a \texttt{READ} reading exactly the same amount of bytes to write. If this is not the case, an \texttt{IllegalStateException} is thrown. Otherwise these previously read bytes are written with direct access to the medium via \IMediumAccessor{}.
\item If action = \texttt{TRUNCATE}: Execute an explicit truncation of the medium.
\item If action = \texttt{INSERT}, \texttt{REPLACE} or \texttt{REMOVE}: Do nothing, as these operations are implicitly implemented via \texttt{READ}, \texttt{WRITE} and \texttt{TRUNCATE} operations.
\end{itemize}
\item If action = \texttt{INSERT}, \texttt{REPLACE} or \texttt{REMOVE}: Call \MediumReferenceRepository{}\texttt{.updateReferences()} for the region, such that all consecutive  \IMediumReference{} instances are updated.
\item Update the cache:
\begin{itemize}
\item If action = \texttt{WRITE}, then call \MediumCache{}\texttt{.addRegion()} to add the bytes written to the cache.
\item If action = \texttt{REMOVE}, then call \MediumCache{}\texttt{.removeRegion()} to remove the bytes also from the cache.
\end{itemize}
\item If action = \texttt{INSERT}, \texttt{REMOVE} or \texttt{REPLACE}: Call \MediumChangeManager{}\texttt{.undo()} to undo the action
\end{enumerate}
