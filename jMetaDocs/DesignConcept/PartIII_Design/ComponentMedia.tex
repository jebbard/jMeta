...
\item If action = \texttt{REPLACE} or \texttt{INSERT}: Do nothing, as these operations are implicitly implemented via \texttt{WRITE} operations.
\item If action = \texttt{REMOVE}: Do nothing, as \texttt{REMOVE} it implicitly gets implemented by \texttt{READ}, \texttt{WRITE} and \texttt{TRUNCATE}.
...
\item Update the cache (part 2): If action = \texttt{WRITE}, \texttt{INSERT} or \texttt{REPLACE}, then call \MediumCache{}\texttt{.addRegion()} to add the bytes to the cache.
