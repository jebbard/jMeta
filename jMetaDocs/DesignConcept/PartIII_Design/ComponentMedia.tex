...
{% Begründung
Why not performing everything within the same loop, i.e. just once iterating the \texttt{MediumAction}s? This would mean to e.g. first execute an \texttt{INSERT} action, and then update all references. The problem: The flush plan might still contain READ and WRITE operations which are referring the offsets on the current medium. Calling \MediumReferenceRepository{}\texttt{.updateReferences()} already now would also update these references, and they would be pointing to the wrong bytes! Thus, we must first process all \texttt{READ} (including adding of the read bytes to the cache), \texttt{WRITE} and \texttt{TRUNCATE} operations with their original offsets. For the caching updates in terms of \texttt{INSERT}, \texttt{REPLACE} And \texttt{REMOVE}, there are additional constraints that make it necessary to do it in the second phase. These are detailled in the next design decision.
}
{% Nachteile
No disadvantages known
}
...
The second part of the answer when to perform \MediumReferenceRepository{}\texttt{.updateReferences()} comes here. Some things need to be kept in mind when updating the cache, which depends on the action type:
%%%% DD --> %%%%
\DD{dd:439g}
{% Titel
...
\begin{itemize}
\item If action = \texttt{INSERT}: First call \MediumReferenceRepository{}\texttt{.updateReferences()} for the action, then add the insertion bytes to the cache. Finally, call \MediumChangeManager{}.\texttt{undo} on the action.
\item If action = \texttt{REMOVE}: First remove the bytes in the removal region from the cache, then call \MediumReferenceRepository{}\texttt{.updateReferences()} for the action. Finally, call \MediumChangeManager{}.\texttt{undo} on the action.
\item If action = \texttt{REPLACE}: First remove the bytes in the replaced region from the cache, then call \MediumReferenceRepository{}\texttt{.updateReferences()} for the action, then add the replacement bytes to the cache. Finally, call \MediumChangeManager{}.\texttt{undo} on the action.
\end{itemize}
\end{enumerate}
