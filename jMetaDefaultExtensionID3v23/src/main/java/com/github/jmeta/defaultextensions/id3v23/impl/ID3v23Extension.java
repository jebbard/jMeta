...
public class ID3v23Extension implements Extension {
	...
	private DataFormatSpecification createSpecification() {
		...
		return builder.addContainerWithContainerBasedPayload("id3v23", "id3v23 tag", "The id3v23 tag")
			...
			.addStringField("id", "id3v23 tag header id", "The id3v23 tag header id").withStaticLengthOf(3)
			.withDefaultValue("ID3").withFixedCharset(Charsets.CHARSET_ISO).asMagicKey().finishField()
			.addBinaryField("version", "id3v23 tag header version", "The id3v23 tag header version")
			.withStaticLengthOf(2).withDefaultValue(id3v23TagVersionBytes).finishField()
			.addFlagsField("flags", "id3v23 tag header flags", "The id3v23 tag header flags").withStaticLengthOf(1)
			...
			.finishField().addNumericField("size", "id3v23 tag size", "The id3v23 tag size").withStaticLengthOf(4)
			.withFieldFunction(new SummedSizeOf(ID3v23Extension.REF_EXT_HEADER, payloadReference))
			.withCustomConverter(ID3v23Extension.SYNC_SAFE_INTEGER_CONVERTER).finishField().finishHeader()
			...
			.withFieldFunction(new SizeOf(ID3v23Extension.REF_EXT_HEADER)).finishField()
			...
			.finishField().addNumericField("crc", "id3v23 extended header CRC", "The id3v23 extended header CRC")
			.referencedAs(crcReference).asOptional().withStaticLengthOf(4).finishField().finishHeader().getPayload()
			...
			.referencedAs(frameReference).asDefaultNestedContainer().withIdField(frameIdReference)
			...
			.withFixedCharset(Charsets.CHARSET_ISO).referencedAs(frameIdReference).finishField()
			.addNumericField("size", "Generic frame size field", "The generic frame size field").withStaticLengthOf(4)
			.withFieldFunction(new SizeOf(framePayloadReference))
			.withCustomConverter(ID3v23Extension.SYNC_SAFE_INTEGER_CONVERTER).finishField()
			.addFlagsField("flags", "Generic frame flags field", "The generic frame flags field").withStaticLengthOf(2)
			...
			.addFlagDescription(
				new FlagDescription(ID3v23Extension.FRAME_FLAGS_READ_ONLY, new BitAddress(0, 2), "", 1, null))
			.addFlagDescription(
				new FlagDescription(ID3v23Extension.FRAME_FLAGS_COMPRESSION, new BitAddress(1, 0), "", 1, null))
			.addFlagDescription(
				new FlagDescription(ID3v23Extension.FRAME_FLAGS_ENCRYPTION, new BitAddress(1, 1), "", 1, null))
			.addFlagDescription(
				new FlagDescription(ID3v23Extension.FRAME_FLAGS_GROUP_IDENTITY, new BitAddress(1, 2), "", 1, null))
			.finishFlagSpecification()
			...
			.finishField().finishHeader().getPayload().referencedAs(framePayloadReference)
			...
			.referencedAs(decompressedSizeReference).withStaticLengthOf(4).asOptional().finishField()
			.addNumericField("encryptionMethod", "Encryption method field", "The encryption method field")
			.referencedAs(encryptionMethodReference).withStaticLengthOf(1).asOptional().finishField()
			.addNumericField("groupId", "Decompressed size field", "The decompressed size field")
			.referencedAs(groupIdReference).withStaticLengthOf(1).asOptional().finishField()
			.addBinaryField("data", "Payload data field", "The payload data field").referencedAs(dataFieldReference)
			.withLengthOf(1, DataBlockDescription.UNDEFINED).finishField().finishFieldBasedPayload().finishContainer()
			.addContainerWithFieldBasedPayload("TEXT_FRAME_ID", "GENERIC_ID3v23_TEXT_FRAME",
				"The id3v23 GENERIC_TEXT_FRAME")
			.cloneFrom(frameReference).referencedAs(textFrameReference).getPayload().withoutField(dataFieldReference)
			.addStringField("textEncoding", "Text encoding", "Text encoding").withStaticLengthOf(1)
			...
			.addEnumeratedValue(new byte[] { 1 }, Charsets.CHARSET_UTF16.name()).finishField()
			.addStringField("information", "Information", "Information").referencedAs(informationReference)
			.withTerminationCharacter('\u0000').withLengthOf(1, DataBlockDescription.UNDEFINED).finishField()
			...
			.cloneFrom(textFrameReference).finishContainer()
			...
			.cloneFrom(textFrameReference).finishContainer()
			.addContainerWithFieldBasedPayload("padding", "Padding", "Padding").referencedAs(paddingReference)
			.addHeader("header", "Padding header", "Padding header")
			.addBinaryField("key", "id3v23 padding header key", "The id3v23 padding header key")
			.withDefaultValue(new byte[] { 0 }).withStaticLengthOf(1).asMagicKey().finishField().finishHeader()
			.getPayload().withDescription("Padding payload", "Padding payload")
			.addBinaryField("bytes", "Padding bytes", "Padding bytes").withDefaultValue(new byte[] { 0 })
			.withLengthOf(0, DataBlockDescription.UNDEFINED).finishField().finishFieldBasedPayload().finishContainer()
			.finishContainerBasedPayload().finishContainer().withByteOrders(ByteOrder.BIG_ENDIAN)
