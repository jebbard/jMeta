...
import de.je.util.common.time.MeasurementCommandListener;
import de.je.util.common.time.MeasurementSession;
import de.je.util.common.time.SystemMillisTimeProvider;
import de.je.util.io.file.FileUtility;
...
public class FilePerformanceRunner implements MeasurementCommandListener {
    ...
    */
   public FilePerformanceRunner(File testFile, int bytesToRead,
      int bytesToWrite, int bytesAtEnd, boolean deleteTempFiles,
      String machineString) {
      Reject.ifNull(testFile, "testFile");
      Reject.ifTrue(!testFile.exists(),
         "The given file " + testFile.getAbsolutePath() + " exists");
      ...
      try {
         ...
         m_accessors = new AbstractFileAccessor[] {
            new FileChannelAccessor(cloneFile(testFile, runDir), writeBytes,
               bytesToRead, bytesAtEnd, deleteTempFiles),
            new StreamAccessor(cloneFile(testFile, runDir), writeBytes,
               bytesToRead, bytesAtEnd, deleteTempFiles),
            new BufferedStreamAccessor(cloneFile(testFile, runDir), writeBytes,
               bytesToRead, bytesAtEnd, deleteTempFiles),
            new MappedByteBufferAccessor(cloneFile(testFile, runDir),
               writeBytes, bytesToRead, bytesAtEnd, deleteTempFiles), };

         m_session = new MeasurementSession(new SystemMillisTimeProvider(),
            machineString);
         m_resultWriter = new ResultsWriter(
            new File("./data/performanceTest/results"));

         m_session.setMeasurementCommandListener(this);
      }

      catch (Exception e) {
         final String message = "Exception occurred during file performance test initialization: "
            + e.getMessage();
         LOG.severe(message);
    ...
    */
   public void aboutToExecuteCommand(MeasuredCommand command,
    ...
    */
   public void commandExecuted(MeasuredCommand command,
      MeasurementSession session,
      ...
      LOG.info("Execution time: "
         + (measuredCommandExceution.getStopTime()
            - measuredCommandExceution.getStartTime())
         + " " + session.getTimeProvider().getUnit());

      try {
         m_resultWriter.writeResults(command, m_testFile, m_bytesToRead,
            m_bytesToWrite, m_bytesAtEnd, session, measuredCommandExceution,
            session.getMachineInfo());
         logMemoryFootprint();
      }

      catch (Exception e) {
         LOG.severe("Unable to write data set to csv file <"
            + m_resultWriter.getFilePath() + ">");
         e.printStackTrace();
      }
   }
   ...
   private File cloneFile(File testFile, File runDir) throws IOException {
      ...
      File clonedFile = new File(runDir,
         m_testFile.getName() + "_" + CLONED_FILE_COUNTER++);

      if (!FileUtility.copyFile(testFile, clonedFile))
         throw new RuntimeException(
            "Could not copy test file: <" + clonedFile.getAbsolutePath() + ">");
   ...
   private File createRunDirectory() {
      ...
      File runDir = new File("./data/performanceTest/runs" + File.separator
         + "RUN_" + sdf.format(currentDate) + "_" + m_testFile.getName());

      if (!runDir.mkdir())
         throw new RuntimeException(
            "Could not create directory for performance run: <"
   ...
   private void checkFile(File testFile) {
      ...
      final PerformanceTestFileFilter filter = new PerformanceTestFileFilter(
         minimumSize);

      if (!filter.accept(testFile)) {
         final String message = "File <" + m_testFile
            + "> cannot be measured due to at least one "
   ...
   private void logMemoryFootprint() {
      ...
      LOG.info(
         "[MEM] allocated memory: " + m_runtime.totalMemory() + " [byte]");
   ...
   static {
      try {
         final FileHandler handler = new FileHandler("." + File.separator
            + "logs" + File.separator + "measurements_%g.log", true);
