...
public abstract class AbstractFileAccessor implements MeasuredCommand {
    ...
    */
   public AbstractFileAccessor(File file, byte[] bytesToWrite, int bytesToRead,
      int bytesAtEnd, boolean deleteFileAfterClose) {
   ...
   /**
    * @see de.je.util.common.time.MeasuredCommand#getUniqueName()
    */
   @Override
   ...
   /**
    * @see de.je.util.common.design.Command#execute()
    */
   public void execute() {

      try {
         // Read the bytes to read
         // Working with the initial length is important, as at least the MappedByteBuffer
         // implementation has already changed the original size within its constructor
         final long bytesToReadOfs = m_initialLength - m_bytesToRead
            - m_bytesAtEnd;
         ...
         write(bytesToReadOfs + m_bytesToWrite.length + m_bytesToRead,
            bytesAtEnd);
      }

      catch (IOException e) {
         throw new RuntimeException(
            "I/O exception occurred during command execution", e);
      }
   }

   /**
    * Closes the underlying file. The file is deleted if this has been requested within the constructor call.
    * 
    * @throws IOException
    *            if an I/O error occurred during close operation.
    */
   ...
   /**
    * Closes the file. This method's implementation depends on the concrete object to close.
    * 
    * @throws IOException
    ...
    *            whenever an I/O operation failed.
    */
   protected abstract void write(long offset, byte[] bytesToWrite)
      throws IOException;
