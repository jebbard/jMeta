...
public abstract class AbstractAPETagSearcher extends AbstractMagicKeyTagSearcher {
	...
	private APETagHeaderInfo getTagHeaderInfo(ByteBuffer bb) {
		...
		if (hasHeader && hasFooter) {
			tagSize += AbstractAPETagSearcher.APE_TAG_HEADER_SIZE;
		}
	...
	protected int getTagRelativeStartOffset(int tagSize) {
		...
		return -tagSize + AbstractAPETagSearcher.APE_TAG_HEADER_SIZE;
	}
	...
	protected int getTotalTagSize(RandomAccessFile file, long possibleOffset) throws IOException {
		...
		ByteBuffer bb = ByteBuffer.allocate(AbstractAPETagSearcher.APE_TAG_HEADER_SIZE);
