...
/**
 * {@link ResultsWriter} writes the performance test results to an external
 * file.
 */
public class ResultsWriter {
	 ...
	 *
	 * @param resultDir the directory the performance test results are stored.
	 * @throws IOException if creation of the result file failed.
	 */
	public ResultsWriter(File resultDir) throws IOException {
		final File csvFile = new File(resultDir, ResultsWriter.CSV_FILE_NAME);
		...
		m_csvWriter = new CsvWriter(ResultsWriter.FILE_HEADING.length);
		...
		if (!appendMode) {
			m_csvWriter.writeNextRow(ResultsWriter.FILE_HEADING);
		}
	}
	 ...
	 *
	 * @throws IOException if closing was not successful
	 */
	...
	/**
	 * Writes the results of a single measurement to the results file as a data
	 * record.
	 *
	 * @param command       The {@link MeasuredCommand}
	 * @param testFile      The test {@link File}
	 * @param bytesToRead   The number of bytes read
	 * @param bytesToWrite  The number of bytes written
	 * @param bytesAtEnd    The number of bytes at EOF
	 * @param session       The {@link MeasurementSession}
	 * @param execution
	 * 
	 * @param machineString The string identifying the machine.
	...
	public void writeResults(MeasuredCommand command, File testFile, long bytesToRead, long bytesToWrite,
		...
		Object[] record = new Object[] { session.getClass().getSimpleName(), session.getSessionId(), machineString,
			ResultsWriter.JAVA_VERSION_INFO, ResultsWriter.OS_PLATFORM_INFO, execution.getStartTime(),
			execution.getStopTime(), execution.getStopTime() - execution.getStartTime(),
			session.getTimeProvider().getUnit(), command.getUniqueName(), testFile.getParentFile().getAbsolutePath(),
			testFile.getName(), testFile.length(), bytesToRead, bytesToWrite, bytesAtEnd, };
