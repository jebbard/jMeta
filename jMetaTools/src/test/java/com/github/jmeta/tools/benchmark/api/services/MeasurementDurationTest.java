...
public class MeasurementDurationTest {
	...
	/**
	 * For better usability, this method creates a {@link MeasurementDuration}
	 * corresponding to the number of days, hours, minutes and so on to create a
	 * duration time.
	 * 
	 * @param targetUnit   The measurement {@link TimeUnit} of the
	 *                     {@link MeasurementDuration} to create.
	 ...
	 * @return The {@link MeasurementDuration} corresponding to the targetUnit and
	 *         the given specific durations.
	 */
	private static MeasurementDuration createMeasurementDuration(TimeUnit targetUnit, int days, int hours, int minutes,
		...
		return new MeasurementDuration(targetUnit.convert(days, TimeUnit.DAYS)
			+ targetUnit.convert(hours, TimeUnit.HOURS) + targetUnit.convert(minutes, TimeUnit.MINUTES)
			+ targetUnit.convert(seconds, TimeUnit.SECONDS) + targetUnit.convert(milliseconds, TimeUnit.MILLISECONDS)
			+ targetUnit.convert(microseconds, TimeUnit.MICROSECONDS)
			+ targetUnit.convert(nanoseconds, TimeUnit.NANOSECONDS), targetUnit);
	}

	/**
	 * Checks the {@link MeasurementDuration} for having the expected whole and
	 * remaining units.
	 * 
	 * @param measurementDur           The {@link MeasurementDuration} to check.
	 * @param expectedWholeDays        Expected whole days
	 * @param expectedWholeHours       Expected whole hours
	 * @param expectedWholeMinutes     Expected whole minutes
	 * @param expectedWholeSeconds     Expected whole seconds
	 * @param expectedRemainingHours   Expected remaining hours
	 * @param expectedRemainingMinutes Expected remaining minutes
	 * @param expectedRemainingSeconds Expected remaining seconds
	 */
	private void checkDefaultDurations(MeasurementDuration measurementDur, long expectedWholeDays,
		...
		Assert.assertEquals(expectedRemainingSeconds, remainingSeconds);
		Assert.assertEquals(expectedWholeSeconds, totalSeconds);
		Assert.assertEquals(expectedRemainingMinutes, remainingMinutes);
		Assert.assertEquals(expectedWholeMinutes, totalMinutes);
		Assert.assertEquals(expectedRemainingHours, remainingHours);
		Assert.assertEquals(expectedWholeHours, totalHours);
		Assert.assertEquals(expectedWholeDays, totalDays);
	}

	/**
	 * Helper method checking {@link MeasurementDuration#format(String)}. For better
	 * usability, it takes the number of days, hours, minutes and so on to create a
	 * duration time. Then it creates a corresponding {@link MeasurementDuration}
	 * with a specified {@link TimeUnit}. And finally it formats the
	 * {@link MeasurementDuration} using
	 * {@link MeasurementDuration#LONG_PATTERN_EN},
	 * {@link MeasurementDuration#SHORT_PATTERN} and
	 * {@link MeasurementDuration#TINY_PATTERN} as well as a custom pattern.
	 * 
	 * The method highly depends upon the structure of each pattern. Thus, if one of
	 * the mentioned patterns changes this method is highly likely to require
	 * changes, too.
	 * 
	 * @param targetUnit   The measurement {@link TimeUnit} of the
	 *                     {@link MeasurementDuration} to create.
	...
	private void checkFormat(TimeUnit targetUnit, int days, int hours, int minutes, int seconds, int milliseconds,
		...
		MeasurementDuration dur = MeasurementDurationTest.createMeasurementDuration(targetUnit, days, hours, minutes,
		...
		final String expectedShortFormat = (days != 0 ? days + " d" : "")
			...
			+ ((milliseconds != 0) && ((microseconds != 0) || (nanoseconds != 0)) ? ", " : "")
			+ (microseconds != 0 ? microseconds + " \u03BCs" : "")
			+ ((microseconds != 0) && (nanoseconds != 0) ? ", " : "") + (nanoseconds != 0 ? nanoseconds + " ns" : "");
		...
		Assert.assertEquals(expectedCompleteFormat, dur.format(MeasurementDurationTest.COMPLETE_PATTERN));
	}

	/**
	 * Creates a new {@link MeasurementDuration} and performs some default checks
	 * with it.
	 * 
	...
	private MeasurementDuration createMeasurementDurationWithDefaultChecks(final long durationTime,
		...
		Assert.assertEquals(measurementDur.getDurationTime(), durationTime);
		Assert.assertEquals(measurementDur.getMeasurementUnit(), unit);
	...
	public void testFormat() {
		...
		MeasurementDuration zeroDur1 = MeasurementDurationTest.createMeasurementDuration(TimeUnit.MICROSECONDS, 0, 0, 0,
			0, 0, 0, 0);
		...
		Assert.assertEquals("0_0_0_0_0_0_0", zeroDur1.format(MeasurementDurationTest.COMPLETE_PATTERN));
	...
	public void testSecondFractions() {
		...
		Assert.assertEquals(1400, md1.getWholeUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(0, md1.getRemainingUnits(TimeUnit.MICROSECONDS));
		Assert.assertEquals(1, md1.getWholeUnits(TimeUnit.SECONDS));

		MeasurementDuration md2 = createMeasurementDurationWithDefaultChecks(1400, TimeUnit.SECONDS);
		Assert.assertEquals(1400000, md2.getWholeUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(1400000000, md2.getWholeUnits(TimeUnit.MICROSECONDS));
		Assert.assertEquals(1400, md2.getWholeUnits(TimeUnit.SECONDS));
		Assert.assertEquals(0, md2.getRemainingUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(0, md2.getRemainingUnits(TimeUnit.MICROSECONDS));
		Assert.assertEquals(20, md2.getRemainingUnits(TimeUnit.SECONDS));

		MeasurementDuration md3 = createMeasurementDurationWithDefaultChecks(60000, TimeUnit.MILLISECONDS);
		Assert.assertEquals(60000, md3.getWholeUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(0, md3.getRemainingUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(60, md3.getWholeUnits(TimeUnit.SECONDS));
		Assert.assertEquals(0, md3.getRemainingUnits(TimeUnit.SECONDS));

		MeasurementDuration md4 = createMeasurementDurationWithDefaultChecks(1400, TimeUnit.MICROSECONDS);
		Assert.assertEquals(1, md4.getWholeUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(1400, md4.getWholeUnits(TimeUnit.MICROSECONDS));
		Assert.assertEquals(1400000, md4.getWholeUnits(TimeUnit.NANOSECONDS));
		Assert.assertEquals(1, md4.getRemainingUnits(TimeUnit.MILLISECONDS));
		Assert.assertEquals(400, md4.getRemainingUnits(TimeUnit.MICROSECONDS));
		Assert.assertEquals(0, md4.getRemainingUnits(TimeUnit.NANOSECONDS));
	}
