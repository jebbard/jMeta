...
/**
 * {@link AbstractEqualsTest} tests for the correct implementation of the
 * {@link #equals(Object)} method, which includes reflexivity, transitivity and
 * symmetry as well as expected behavior of {@link #hashCode()}.
 *
 * @param <T> The type of object to be equality-tested.
 */
public abstract class AbstractEqualsTest<T> {
	 ...
	 *
	 * @throws IllegalArgumentException If data is not valid with a speaking message
	...
	public void equals_isReflexive_forTheIdenticalObjects() {
		...
		for (int i = 0; i < objects.size(); ++i) {
			...
			Assert.assertThat(object.equals(object), CoreMatchers.is(true));
		}
	}
	...
	/**
	 * Returns right objects that are known to be unequal to the corresponding left
	 * object with the same index.
	 *
	 * @return Right objects that are known to be unequal to the corresponding left
	 *         object.
	 */
	protected abstract List<T> getDifferentObjects();

	/**
	 * Returns right objects to be used in equals test. The right objects are tested
	 * to be unequal to null, equal to themselves and equal to the left objects with
	 * the same index.
	 *
	...
	/**
	 * Returns left objects to be used in equals test. The right objects are tested
	 * to be unequal to null, equal to themselves and equal to the right objects
	 * with the same index.
	 *
	...
	/**
	 * Returns third objects that are known to be equal to the right objects with
	 * the same index.
	 *
	 * @return Third objects that are known to be equal to the right objects with
	 *         the same index.
	 */
