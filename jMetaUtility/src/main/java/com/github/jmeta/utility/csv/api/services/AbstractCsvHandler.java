...
/**
 * {@link AbstractCsvHandler} provides general methods for a class that is able
 * to read or write csv (comma separated value) files.
 *
 * @param <T> The concrete type of closable class
 */
public abstract class AbstractCsvHandler<T extends Closeable & AutoCloseable> {

	/**
	 * The default separator that is used initially by a new {@link CsvReader}
	 * instance or if null is passed to {@link #setSeparator(Character)}
	 */
	public static final Character DEFAULT_SEPARATOR = ';';
	/**
	 * The default quote that is used initially by a new {@link CsvReader} instance
	 * or if null is passed to {@link #setQuote(Character)}
	 */
	...
	/**
	 * Closes the current file of this {@link CsvReader}. A file must be currently
	 * loaded to be able to call this method. The {@link CsvReader} can further be
	 * used after this method has been called. For this, simply a new source must be
	 * set using {@link #setNewResource(Closeable)}.
	 * 
	 * As a precondition, the CSV must be loaded.
	 * 
	 * @throws IOException If closing the current file failed.
	 */
	 ...
	 * @throws IOException if there was any error during closing the previous
	 *                     resource
	 */
	...
	/**
	 * Sets the quote to use, may be null to indicate the use of the
	 * {@link #DEFAULT_QUOTE}.
	 *
	 * @param quote the quote to use, may be null to indicate the use of the
	 *              {@link #DEFAULT_QUOTE}.
	 */
	public void setQuote(Character quote) {
		if (quote == null) {
			this.quote = AbstractCsvHandler.DEFAULT_QUOTE;
		} else {
			this.quote = quote;
		}
	}
	...
	/**
	 * Sets the separator to use, may be null to indicate the use of the
	 * {@link #DEFAULT_SEPARATOR}.
	 *
	 * @param separator the separator to use, may be null to indicate the use of the
	 *                  {@link #DEFAULT_SEPARATOR}.
	 */
	public void setSeparator(Character separator) {
		if (separator == null) {
			this.separator = AbstractCsvHandler.DEFAULT_SEPARATOR;
		} else {
