...
public class CsvWriter extends AbstractCsvHandler<NamedWriter> {
	 ...
	 * 
	 * @param elementCount The number of elements in a record. Must be at least 1.
	 */
	...
	/**
	 * Appends a new data set to the CSV file using the given data. The string
	 * representation of each object is written in the same order to the CSV file
	 * using its toString() method.
	 *
	 * @param columns The record to append, each object represents an element of the
	 *                record; it must have the same length as returned by
	 *                {@link #getColumnCount()}
	...
	/**
	 * Appends an element to the current CSV data set, i.e. to the given
	 * {@link StringBuilder}. The element string representation is taken using its
	 * toString() method. The string representation is automatically quoted if it
	 * contains a separator character. The element is appended with a separator
	 * character.
	 *
	 * @param builder  The {@link StringBuilder} to append the element to.
	 * @param toAppend The element to be appended.
	 */
