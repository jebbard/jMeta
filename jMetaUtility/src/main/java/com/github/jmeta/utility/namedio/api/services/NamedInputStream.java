...
/**
 * The {@link NamedInputStream} class is a {@link FilterInputStream} that adds
 * some meta data to a given arbitrary, wrapped {@link InputStream}. This is
 * mainly useful for identification purposes, because you sometimes want to know
 * the original source of a given input stream (file, URL etc.).
 */
public class NamedInputStream extends FilterInputStream {

	/**
	 * Creates a new {@link NamedInputStream} instance using a given existing file.
	 * The wrapped {@link InputStream} is created as a {@link FileInputStream}.
	 * 
	 * @param file The given {@link File}. Must exist and be a file.
	 * @return The {@link NamedInputStream}.
	 * @throws IOException if creation of the wrapped {@link InputStream} fails.
	 */
	...
	/**
	 * Creates a new {@link NamedInputStream} instance using a given resource. It
	 * creates a new wrapped {@link InputStream} using the
	 * {@link Class#getResourceAsStream(String)} method. The method appends the
	 * canonical class name to the resource name and uses this as the name of the
	 * {@link NamedInputStream}.
	 * 
	 * @param clazz    The given {@link Class}. The resource must be located
	 *                 relative to the class as described in the
	...
	/**
	 * Creates a new {@link NamedInputStream} instance using a given url. It creates
	 * a new wrapped {@link InputStream} using the {@link URL#openStream()} method.
	 * 
	 ...
	 * @param name Name of the wrapped {@link InputStream}, arbitrary, but non-null.
