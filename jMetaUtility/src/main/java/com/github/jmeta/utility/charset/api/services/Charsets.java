...
/**
 * {@link Charsets} is a helper class defining constants for all
 * {@link Charset}s Java defines as always available.
 *
 * Additionally it provides helper methods to deal with BOMs and figure out
...
public final class Charsets {
	...
	static {
		Charsets.BOMMED_CHARSETS.put(Charsets.CHARSET_UTF16, new byte[] { (byte) 0xfe, (byte) 0xff });
	}

	/**
	 * Returns a byte order mark (a sequence of bytes) if the given charset is a
	 * BOMMED charset, null otherwise.
	 *
	 * @param cs The {@link Charset} to check
	 * @return the byte order mark bytes for the given charset if it is a BOMMED
	 *         charset, null otherwise
	 */
	public static byte[] getBOM(Charset cs) {
		Reject.ifNull(cs, "cs");

		return Charsets.BOMMED_CHARSETS.get(cs);
	}

	/**
	 * Returns a byte representation of a {@link String} encoded in the given
	 * {@link Charset} without BOM (byte order management) bytes which are usually
	 * contained when using {@link String#getBytes(String)}. For instance, all
	 * UTF-16 encoded strings contain the bytes -2 and -1 at the beginning (FEFF or
	 * FFEF, depending on byte order).
	 *
	 * This is necessary to identify the byte ordering. However, in some cases a
	 * String needs to be converter without BOM e.g. when appending raw bytes to an
	 * already BOMed byte representation of a string.
	 *
	 * @param string The string to get the bytes for.
	 * @param cs     The {@link Charset} the string is encoded in.
	 *
	 * @return A byte representation of the string according to the given
	 *         {@link Charset} without BOM (byte order management) bytes.
	 */
	public static byte[] getBytesWithoutBOM(String string, Charset cs) {
		...
		if (Charsets.getBOM(cs) != null) {
			byte[] bomBytes = Charsets.BOMMED_CHARSETS.get(cs);
