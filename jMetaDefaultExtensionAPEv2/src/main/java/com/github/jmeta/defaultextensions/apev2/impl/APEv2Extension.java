...
public class APEv2Extension implements Extension {
	...
	*/
	...
	private DataFormatSpecification createSpecification() {
		...
		// NOTE: Although the APEv2 tag header and footer are optional (header is
		// mandatory if tag is at beginning of
		// file,
		// footer is mandatory if at end of file), we do not declare both as optional
		// because there is no clear criterion.
		// This way, APEv2 tags are only recognized if they have a header (when forward
		// reading) or if they have a footer
		...
		return builder.addContainerWithContainerBasedPayload("apev2", "APEv2 Tag", "The APEv2 Tag")
			.addHeader("header", "APEv2 header", "The APEv2 header").referencedAs(headerReference)
			.addStringField("preample", "APEv2 header preample", "APEv2 header preample")
			.withStaticLengthOf(preampleByteLength).withDefaultValue(apeMagicKeyString).asMagicKey().finishField()
			.addNumericField("versionNumber", "APEv2 header version number", "APEv2 header version number")
			.withStaticLengthOf(4).withDefaultValue(Long.valueOf(0x2000)).finishField()
			.addNumericField("tagSize", "APEv2 header tag size", "APEv2 header tag size").withStaticLengthOf(4)
			.withFieldFunction(new SummedSizeOf(payloadReference, footerReference)).finishField()
			.addNumericField("itemCount", "APEv2 header item count", "APEv2 header item count").withStaticLengthOf(4)
			.withFieldFunction(new CountOf(itemReference)).finishField()
			.addFlagsField("tagFlags", "APEv2 header tag flags", "APEv2 header tag flags").withStaticLengthOf(4)
			.withFlagSpecification(4, ByteOrder.LITTLE_ENDIAN).withDefaultFlagBytes(new byte[4])
			.addFlagDescription(readOnlyFlag).addFlagDescription(itemTypeFlag).addFlagDescription(thisIsTheHeaderFlag)
			.addFlagDescription(tagContainsNoFooterFlag).addFlagDescription(tagContainsHeaderFlag)
			.finishFlagSpecification()
			.withFieldFunction(new PresenceOf(footerReference, tagContainsNoFooterFlag.getFlagName(), 0)).finishField()
			.addBinaryField("reserved", "APEv2 header reserved", "APEv2 header reserved").withStaticLengthOf(8)
			.withDefaultValue(new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 }).finishField().finishHeader().getPayload()
			.referencedAs(payloadReference).withDescription("APEv2 payload", "The APEv2 payload")
			.addGenericContainerWithFieldBasedPayload("ITEM_ID", "APEv2 item", "The APEv2 item")
			.referencedAs(itemReference).asDefaultNestedContainer().withIdField(itemKeyReference)
			.addHeader("header", "APEv2 item header", "The APEv2 item header")
			.addNumericField("size", "APEv2 item value size", "APEv2 item value size").withStaticLengthOf(4)
			.withFieldFunction(new SizeOf(itemPayloadReference)).finishField()
			.addFlagsField("flags", "APEv2 item flags", "APEv2 item flags").withStaticLengthOf(4)
			.withFlagSpecification(4, ByteOrder.LITTLE_ENDIAN).withDefaultFlagBytes(new byte[4])
			.addFlagDescription(readOnlyFlag).addFlagDescription(itemTypeFlag).finishFlagSpecification().finishField()
			.addStringField("key", "APEv2 item key", "APEv2 item key").withLengthOf(2, 255)
			.withTerminationCharacter('\u0000').referencedAs(itemKeyReference).finishField().finishHeader().getPayload()
			.referencedAs(itemPayloadReference).withDescription("APEv2 item payload", "The APEv2 item payload")
			...
			.finishContainerBasedPayload().addFooter("footer", "APEv2 footer", "The APEv2 footer")
			.referencedAs(footerReference).addStringField("preample", "APEv2 footer preample", "APEv2 footer preample")
			.withStaticLengthOf(preampleByteLength).withDefaultValue(apeMagicKeyString).asMagicKey().finishField()
			.addNumericField("versionNumber", "APEv2 footer version number", "APEv2 footer version number")
			.withStaticLengthOf(4).withDefaultValue(Long.valueOf(0x2000)).finishField()
			.addNumericField("tagSize", "APEv2 footer tag size", "APEv2 footer tag size").withStaticLengthOf(4)
			.withFieldFunction(new SummedSizeOf(headerReference, payloadReference)).finishField()
			.addNumericField("itemCount", "APEv2 footer item count", "APEv2 footer item count").withStaticLengthOf(4)
			.withFieldFunction(new CountOf(itemReference)).finishField()
			.addFlagsField("tagFlags", "APEv2 footer tag flags", "APEv2 footer tag flags").withStaticLengthOf(4)
			.withFlagSpecification(4, ByteOrder.LITTLE_ENDIAN).withDefaultFlagBytes(new byte[4])
			.addFlagDescription(readOnlyFlag).addFlagDescription(itemTypeFlag).addFlagDescription(thisIsTheHeaderFlag)
			.addFlagDescription(tagContainsNoFooterFlag).addFlagDescription(tagContainsHeaderFlag)
			.finishFlagSpecification()
			.withFieldFunction(new PresenceOf(headerReference, tagContainsHeaderFlag.getFlagName(), 1)).finishField()
			.addBinaryField("reserved", "APEv2 footer reserved", "APEv2 footer reserved").withStaticLengthOf(8)
			.withDefaultValue(new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 }).finishField().finishFooter().finishContainer()
			.withByteOrders(ByteOrder.LITTLE_ENDIAN).withCharsets(Charsets.CHARSET_ISO).build();
		// @formatter:on
	}
